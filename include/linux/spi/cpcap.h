...
struct cpcap_ind_chrg {
	...
	u8 gpio_chrgterm;
	u8 gpio_chrgcmpl;
	void (*force_charge_complete)(int closed);
	void (*force_charge_terminate)(int closed);
	void (*force_inductive_path)(int closed);
	void (*force_cable_path)(int closed);
	int (*check_inductive_path)(void);
	int (*check_cable_path)(void);
};

struct cpcap_factory_kill {
	u8 supported;
	u8 gpio_ctrl;
	void (*force_factory_kill)(int enabled);
};

struct  hdmi_hpd_data {
	u8 hpd_gpio;
	char (*check_hpd_status)(void);
};
...
#ifdef __KERNEL__
struct cpcap_usb_mux {
	u8 supported;
	u8 bp_bypass_supported;
	u8 gpio_mux_sel1;
	u8 gpio_mux_sel2;
	void (*configure_otg_muxmode)(enum cpcap_usb_otg_muxmode muxmode);
	void (*configure_switch_muxmode)(enum cpcap_usb_switch_muxmode muxmode);
};

struct cpcap_spdif_audio {
	u8 supported;
	u8 gpio_spdif;
	void (*enable_spdif_audio)(int status);
};

struct cpcap_usbhs_ext_pwr {
	u8 supported;
	u8 gpio_usbhs_ext_pwr;
	int gpio_oc;
	/*
	 * function to initialize platform support for
	 *   usb host power
	 */
	int  (*init)(struct cpcap_usbhs_ext_pwr *pdata);

	/*
	 * function to exit from platform support for
	 *   usb host power
	 */
	void (*exit)(struct cpcap_usbhs_ext_pwr *pdata);

	void (*usbhs_ext_pwr_en)(int val);
};
#endif
...
#define CPCAP_IOCTL_ACCY_CHARGER \
	_IOW(0, CPCAP_IOCTL_NUM_ACCY_CHARGER, struct cpcap_whisper_request*)
...
extern void cpcap_set_bit(enum cpcap_reg reg, unsigned short value,
		      unsigned short mask);
