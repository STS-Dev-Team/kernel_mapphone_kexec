...
{
	/* Disable PLL_UNLOCK_INTR */
	if (val == HDMI_PLLPWRCMD_ALLOFF)
		REG_FLD_MOD(HDMI_WP, HDMI_WP_IRQENABLE_CLR, 1, 30, 30);
...
static int hdmi_syslink_notifier_call(unsigned long val, void *v)
{
	int status = 0;
	u16 *proc_id = (u16 *)v;
	switch ((int)val) {
