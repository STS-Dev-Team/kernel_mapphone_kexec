...
static int _enable_clocks(struct omap_hwmod *oh)
{
	...
	/* Check for dependent opt clks */
	for (i = oh->opt_clks_cnt, oc = oh->opt_clks; i > 0; i--, oc++) {
		if (oc->flags & OMAP_DEPENDENT_CLK) {
			pr_debug("omap_hwmod: enabling dependent opt clk %s\n",
								 oc->clk);
			clk_enable(oc->_clk);
		}
	}
	...
	/* Rest of the opt clocks are controlled by the device driver. */
...
static int _disable_clocks(struct omap_hwmod *oh)
{
	...
	/* Check fo dependent opt clks */
	for (i = oh->opt_clks_cnt, oc = oh->opt_clks; i > 0; i--, oc++) {
		if ((oc->flags & OMAP_DEPENDENT_CLK) && (oc->_clk)) {
			pr_debug("omap_hwmod: disabling dependent opt clock %s\n",
									 oc->clk);
			clk_disable(oc->_clk);
		}
	}
 ...
 * Some IP like McBSP does have a reset control but no reset status.
 */
...
int _omap_hwmod_enable(struct omap_hwmod *oh)
{
	...
	if (!r) {
		...
		_disable_clocks(oh);
		WARN(1, "omap_hwmod: %s: _wait_target_ready: %d sysc: %08x\n",
			 oh->name, r, oh->_sysc_cache);
	}
