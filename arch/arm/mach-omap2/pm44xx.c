...
static void enable_suspend_wakeup_sources(void)
{
	/* This is a temp workaround. We are wating for the
	framework of IO wakeup */
	omap_writel(1<<30 | omap_readl(0x4a31e058), 0x4a31e058);
}

static void disable_suspend_wakeup_sources(void)
{
	u32 val;

	/* This is a temp workaround. We are wating for the
	framework of IO wakeup */
	val = omap_readl(0x4a31e058);
	val &= ~(1<<30);
	omap_writel(val, 0x4a31e058);
}
...
static int omap4_pm_suspend(void)
{
	...
	if (enable_off_mode) {
		...
		omap_voltage_channel_switch(OMAP_VOLTAGE_VC_CHNL_CMD);
	}
...
restore:
	...
	disable_suspend_wakeup_sources();
#ifdef CONFIG_PM_DEBUG
	fail = 0;
#endif
...
#ifdef CONFIG_PM_DEBUG
		if (state == PWRDM_POWER_ON && !fail)
			fail = 1;
#endif
	}
#ifdef CONFIG_PM_DEBUG
	if (fail) {
		pm_dbg_regset_dump(1, 3);
		pm_dbg_regset_dump(2, 4);
	}
#endif
...
void omap4_pm_off_mode_enable(int enable)
{
	...
	list_for_each_entry(pwrst, &pwrst_list, node) {
		...
		/*
		 * do not change the state of dss_pwrdm, next_state
		 * for dss_pwrdm is only set for suspend path
		 */
