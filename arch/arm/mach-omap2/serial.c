...
static void omap_uart_save_context(struct omap_uart_state *uart)
{
	...
	if (!regset_save_on_suspend || !enable_off_mode)
		return;
...
static void __omap_uart_block_sleep(struct omap_uart_state *uart)
{
			...
			struct omap_uart_port_info *platform_data)
{
	...
	omap_up.board_uart_probe = platform_data->board_uart_probe;
	omap_up.board_uart_remove = platform_data->board_uart_remove;
	omap_up.board_wake_peer = platform_data->board_wake_peer;

	omap_up.ctsrts = platform_data->ctsrts;
	...
	omap_up.cts_padconf_mode = platform_data->cts_padconf_mode;
	omap_up.cts_padvalue = 0;
	omap_up.console_port = platform_data->console_port;
...
void __init omap_serial_init(struct omap_uart_port_info *platform_data)
{
	...
	/* The Platform Specif Initialisations come from the baord file
