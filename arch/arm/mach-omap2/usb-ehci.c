					...
					*port_mode)
{
	...
	case OMAP_OHCI_PORT_MODE_TLL_6PIN_DPDM:
		...
		/* Fall Through to enable wakeup on other pins */
					...
					*port_mode)
{
	...
	case OMAP_OHCI_PORT_MODE_TLL_6PIN_DPDM:
		...
		/* Fall Through to disable wakeup on other pins */
...
static int usbhs_4430ohci_io_wakeup_event(const enum usbhs_omap3_port_mode
					*port_mode)
{
	u16 reg = 0;
	 ...
	 * This function will be later modified to use MUX framework API's.
	 */
	switch (port_mode[0]) {
	case OMAP_OHCI_PORT_MODE_PHY_6PIN_DATSE0:
	case OMAP_OHCI_PORT_MODE_PHY_6PIN_DPDM:
	...
	case OMAP_OHCI_PORT_MODE_TLL_6PIN_DPDM:
		...
		/* Fall Through to check wakeups on other pins */
...
void usbhs_wakeup()
{
	...
	if (test_bit(USBHS_OHCI_LOADED, &omap->event_state) &&
		...
		(pdata->ohci_phy_suspend == NULL)) {

		/* check for the ohci wakeup */

		if (usbhs_4430ohci_io_wakeup_event(pdata->port_mode)) {
...
static int usbhs_enable(struct uhhtll_hcd_omap *omap, int do_init)
{
	...
	if (cpu_is_omap44xx()) {
		...
		} else if (pdata->port_mode[0] != OMAP_USBHS_PORT_MODE_UNUSED) {
		...
		} else if (pdata->port_mode[1] != OMAP_USBHS_PORT_MODE_UNUSED) {
			/* OHCI serial modes. HSIC is not yet supported */
...
static void usbhs_ehci_devoff_suspend(struct uhhtll_hcd_omap *omap)
{
	...
	if (portmode[0] == OMAP_EHCI_PORT_MODE_PHY ||
		portmode[1] == OMAP_EHCI_PORT_MODE_PHY) {
			usbhs_phy_safe = 1;
...
static int uhhtll_drv_suspend(enum driver_type drvtype)
{
	...
	if ((drvtype == OMAP_EHCI) &&
		...
		if (!test_bit(USBHS_OHCI_SUSPENED, &omap->event_state)) {
			/*
			 *  Turn on IO Wakeups only if Transceiver is not
			 * powered off.
			 */
			if (pdata->ohci_phy_suspend == NULL)
	...
	if ((drvtype == OMAP_EHCI) &&
		...
		if (test_and_clear_bit(USBHS_OHCI_SUSPENED,
			...
			if (pdata->ohci_phy_suspend) {
				...
				/*
				 * Add a 100 usec delay for the Transceiver
				 * Timing to settle down. Otherwise in some
				 * cases a USB reset is seen after turning on
				 * the USB Controller.
				 */
				udelay(100);
			}
...
static void setup_4430ehci_drvstrength(const enum usbhs_omap3_port_mode
					*port_mode)
{
	u32 reg;
...
void __init usb_uhhtll_init(const struct usbhs_omap_platform_data *pdata)
{
	...
	} else if (!machine_is_mapphone() && cpu_is_omap44xx()) {
