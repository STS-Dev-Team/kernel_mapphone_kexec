 ...
 */
#define GPIO_SIGNAL_SD_DET_N 106
...
int __init mapphone_hsmmc_init(void)
{
	...
	if (mmc_node) {
		...
		if (mmc_prop && (*(int *)mmc_prop == MMC_CARD_CONNECT_SDIO)) {
			mmc_prop = of_get_property(mmc_node,
				DT_PROP_MMC_SDCNUM, NULL);
			if (mmc_prop) {
	...
	if (!is_found)
		tiwlan_mmc_controller = CONFIG_TIWLAN_MMC_CONTROLLER;
#else
	tiwlan_mmc_controller = CONFIG_TIWLAN_MMC_CONTROLLER;
#endif
	if (tiwlan_mmc_controller > 0 &&
			tiwlan_mmc_controller <= OMAP44XX_NR_MMC)
