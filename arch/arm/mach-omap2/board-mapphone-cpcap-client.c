...
static struct platform_device cpcap_bq24618 = {
	.name           = "bq24618",
	.id             = -1,
	.dev            = {
		.platform_data  = NULL,
...
static struct cpcap_bt_led_config_data bt_led_data = {
	.init = CPCAP_BT_INIT,
	.on = CPCAP_BT_ON,
	.class_name = CPCAP_BT_LED_CLASS_NAME,
};

static struct platform_device cpcap_bt_led = {
	.name = CPCAP_BT_LED_DRV_NAME,
	.id   = -1,
	.dev  = {
		.platform_data = &bt_led_data,
	},
};

static struct cpcap_cp_led_config_data cp_led_data = {
	.init = CPCAP_CP_INIT,
	.on = CPCAP_CP_ON,
	.class_name = CPCAP_CP_LED_CLASS_NAME,
};

static struct platform_device cpcap_cp_led = {
	.name = CPCAP_CP_LED_DRV_NAME,
	.id   = -1,
	.dev  = {
		.platform_data = &cp_led_data,
	},
};
...
static struct cpcap_rgb_led_config_data rgb_led_data = {
	.low_value = CPCAP_RGB_LOW_VALUE,
	.low_med_value = CPCAP_RGB_LOW_MED_VALUE,
	.med_value = CPCAP_RGB_MEDIUM_VALUE,
	.med_high_value = CPCAP_RGB_MED_HIGH_VALUE,
	.high_value = CPCAP_RGB_HIGH_VALUE,
	.red_enable = true,
	.green_enable = true,
	.blue_enable = true,
	.blink_enable = true,
...
static struct gpio_led generic_gpio_led_info = {
	.name = "kpd_backlight_en",
	.default_trigger = "",
	.gpio = 38,
	.active_low = 0,
...
static struct gpio_led_platform_data generic_gpio_data = {
	.num_leds = 1,
	.leds = &generic_gpio_led_info,
	.gpio_blink_set = NULL,
};

static struct platform_device generic_gpio = {
	.name = "leds-gpio",
	.id   = -1,
	.dev  = {
		.platform_data  = &generic_gpio_data,
	},
};
...
static void __init charger_cpcap_extchrg_ic_init(void)
{
	struct device_node *node;
	const void *prop;
	int size;

	pr_info("External charger ic init\n");

	node = of_find_node_by_path(DT_PATH_EXCHARGER);
	if (node != NULL) {
		...
		if (prop && size) {
			switch (*(int *)prop) {
			case 0x00030012: /* BQ24618 */
				pr_info("BQ24618 device added\n");
				cpcap_device_register(&cpcap_bq24618);
				break;
...
static int cpcap_bt_led_init(struct device_node *node)
{
	const void *prop;
	int len = 0;
	...
	prop = of_get_property(node, "init", &len);
	if (prop && len)
		bt_led_data.init = *(u16 *)prop;

	prop = of_get_property(node, "on", &len);
	if (prop && len)
		bt_led_data.on = *(u16 *)prop;

	prop = of_get_property(node, "dev_name", &len);
	if (prop && len) {
		strncpy(bt_led_data.class_name, (char *)prop,
			sizeof(bt_led_data.class_name) - 1);
		bt_led_data.class_name[sizeof(bt_led_data.class_name) - 1]
			= '\0';
	}

	cpcap_device_register(&cpcap_bt_led);

	return 0;
}

static int cpcap_cp_led_init(struct device_node *node)
{
	const void *prop;
	int len = 0;
	...
	prop = of_get_property(node, "init", &len);
	if (prop && len)
		cp_led_data.init = *(u16 *)prop;

	prop = of_get_property(node, "on", &len);
	if (prop && len)
		cp_led_data.on = *(u16 *)prop;

	prop = of_get_property(node, "dev_name", &len);
	if (prop && len) {
		strncpy(cp_led_data.class_name, (char *)prop,
			sizeof(cp_led_data.class_name) - 1);
		cp_led_data.class_name[sizeof(cp_led_data.class_name) - 1]
			= '\0';
	}

	cpcap_device_register(&cpcap_cp_led);
...
static int cpcap_rgb_led_init(struct device_node *node)
{
	...
	prop = of_get_property(node, "low_value", &len);
	if (prop && len)
		rgb_led_data.low_value = *(u16 *)prop;

	prop = of_get_property(node, "low_med_value", &len);
	if (prop && len)
		rgb_led_data.low_med_value = *(u16 *)prop;

	prop = of_get_property(node, "med_value", &len);
	if (prop && len)
		rgb_led_data.med_value = *(u16 *)prop;

	prop = of_get_property(node, "med_high_value", &len);
	if (prop && len)
		rgb_led_data.med_high_value = *(u16 *)prop;

	prop = of_get_property(node, "high_value", &len);
	if (prop && len)
		rgb_led_data.high_value = *(u16 *)prop;
...
static int generic_gpio_init(struct device_node *node)
{
	const void *prop;
	int len = 0;
	int gpio;
	...
	if (prop && len) {
		len = sizeof(generic_gpio_led_info.regulator);
		strncpy(generic_gpio_led_info.regulator, (char *)prop, len - 1);
		generic_gpio_led_info.regulator[len - 1] = '\0';
		pr_info("%s: regulator=%s\n", __func__,
			generic_gpio_led_info.regulator);
	}

	gpio = get_gpio_by_name(generic_gpio_led_info.name);
	if (gpio < 0)
		pr_err("%s: Cannot retrieve %s from device tree\n",
			__func__, generic_gpio_led_info.name);
	else {
		generic_gpio_led_info.gpio = gpio;
		pr_info("%s: %s = gpio %d\n", __func__,
			generic_gpio_led_info.name, gpio);
	}

	ret = platform_device_register(&generic_gpio);
	...
	while (node) {
		...
		if (prop && len) {
			...
			case 0x00210005:
				...
				break;
			case 0x00210006:
				pr_info("CPCAP BT found.\n");
				cpcap_bt_led_init(node);
				break;
			case 0x00210007:
				pr_info("CPCAP CP found.\n");
				cpcap_cp_led_init(node);
				break;
			case 0x00000014:
				pr_info("Generic GPIO found.\n");
				generic_gpio_init(node);
				break;
			default:
				pr_err("Unknown CPCAP LED device 0x%08x.\n",
					*(int *)prop);
				break;
			}
		}
