...
#define VP_TRANXDONE_TIMEOUT	1000
...
static struct omap_volt_data omap44xx_vdd_core_volt_data[] = {
	...
	{.volt_nominal = 1125000, .sr_errminlimit = 0xF9, .vp_errgain = 0x16},
};
...
static struct omap_vdd_dep_volt omap44xx_vddmpu_vddcore_data[] = {
	...
	{.main_vdd_volt = 1200000, .dep_vdd_volt = 1125000},
	{.main_vdd_volt = 1313000, .dep_vdd_volt = 1125000},
	{.main_vdd_volt = 1375000, .dep_vdd_volt = 1125000},
	{.main_vdd_volt = 0, .dep_vdd_volt = 0},
};
...
static struct omap_vdd_dep_volt omap44xx_vddiva_vddcore_data[] = {
	...
	{.main_vdd_volt = 1188000, .dep_vdd_volt = 1125000},
	{.main_vdd_volt = 1300000, .dep_vdd_volt = 1125000},
	{.main_vdd_volt = 0, .dep_vdd_volt = 0},
};
...
static int sr_conv_volt_debug_get(struct seq_file *s, void *unused)
{
	int i;
	struct omap_volt_data *volt_data;
	struct omap_vdd_info  *vdd = (struct omap_vdd_info *)s->private;

	if (!vdd || !vdd->volt_data || !vdd->volt_data_count) {
		seq_printf(s, "vdd or volt_data or volt_data_count invalid\n");
		return 0;
	}

	seq_printf(s, "vdd name: %s\n", (vdd->voltdm).name);
	for (i = 0; i < vdd->volt_data_count; i++) {
		seq_printf(s, "\tvolt_nominal = %d, volt_converged = %d\n",
				vdd->volt_data[i].volt_nominal,
				vdd->volt_data[i].volt_converged);
	}
	seq_printf(s, "\n");
	return 0;
}

static int sr_conv_volt_debug_open(struct inode *inode, struct file *file)
{
	return single_open(file, sr_conv_volt_debug_get, inode->i_private);
}

static const struct file_operations sr_conv_volt_debug_fops = {
	.open           = sr_conv_volt_debug_open,
	.read           = seq_read,
	.llseek         = seq_lseek,
...
static void __init omap4_init_voltagecontroller(void)
{
	...
	omap_voltage_channel_switch(OMAP_VOLTAGE_VC_CHNL_VLT);
...
#endif
	(void) debugfs_create_file("converged_volts", S_IRUGO, vdd_debug,
				(void *) vdd, &sr_conv_volt_debug_fops);
		...
		unsigned long target_volt)
{
	...
	if (timeout >= VP_TRANXDONE_TIMEOUT) {
		pr_err("%s: vdd_%s TRANXDONE timeout exceeded."
			...
			"  vpconfig=0x%x, irqst=0x%x vpstatus=0x%x\n"
			, __func__, vdd->voltdm.name,
			voltage_read_reg(vdd->vp_offs.vpconfig),
			prm_read_mod_reg(ocp_mod, prm_irqst_reg_offs),
			voltage_read_reg(vdd->vp_offs.vstatus));
		dump_stack();
	}
...
void __init omap_voltage_init_vc(struct omap_volt_vc_data *setup_vc)
{
	...
	vc_config.vdd0_oncmd = setup_vc->vdd0_oncmd;
	vc_config.vdd0_offcmd = setup_vc->vdd0_offcmd;
	...
	vc_config.vdd1_oncmd = setup_vc->vdd1_oncmd;
	vc_config.vdd1_offcmd = setup_vc->vdd1_offcmd;
	...
	vc_config.vdd2_oncmd = setup_vc->vdd2_oncmd;
	vc_config.vdd2_offcmd = setup_vc->vdd2_offcmd;
}
...
int omap_voltage_scale(struct voltagedomain *voltdm, unsigned long volt)
{
	...
	volt_data = omap_voltage_get_voltdata(voltdm, curr_volt);
	if (IS_ERR(volt_data)) {
		pr_warning("%s: Unable to get volt table for vdd_%s",
			__func__, vdd->voltdm.name);

	} else {
		volt_data->volt_converged = omap_vp_get_curr_volt(voltdm);
	}
...
/**
 * omap_voltage_channel_switch - API for Voltage Channel Switching
 * @chnl : 0 - voltage, 1 - command
  */
int omap_voltage_channel_switch(enum omap_voltage_vc_chnl chnl)
{
	u32 value;
	u8 on_cmd, off_cmd;
	u8 impu, icore, iiva;
	...
	if ((vdd_info[impu].pmic->vc_offmode_chnl_cmd == 0) &&
		(vdd_info[icore].pmic->vc_offmode_chnl_cmd == 0) &&
		(vdd_info[iiva].pmic->vc_offmode_chnl_cmd == 0))
		return 0;

	value = voltage_read_reg(OMAP4_PRM_VC_CFG_CHANNEL_OFFSET);

	if (chnl == OMAP_VOLTAGE_VC_CHNL_VLT) {
		if (vdd_info[impu].pmic->vc_offmode_chnl_cmd != 0)
			value = value & ~OMAP4430_RACEN_VDD_MPU_L_MASK &
				~OMAP4430_RAC_VDD_MPU_L_MASK;
		if (vdd_info[icore].pmic->vc_offmode_chnl_cmd != 0)
			value = value &	~OMAP4430_RACEN_VDD_CORE_L_MASK &
				~OMAP4430_RAC_VDD_CORE_L_MASK;
		if (vdd_info[iiva].pmic->vc_offmode_chnl_cmd != 0)
			value = value &	~OMAP4430_RACEN_VDD_IVA_L_MASK &
				~OMAP4430_RAC_VDD_IVA_L_MASK;
		voltage_write_reg(OMAP4_PRM_VC_CFG_CHANNEL_OFFSET, value);

		if (vdd_info[impu].pmic->vc_offmode_chnl_cmd != 0) {
			value = voltage_read_reg(
				OMAP4_PRM_VC_VAL_CMD_VDD_MPU_L_OFFSET);
			on_cmd = vdd_info[impu].pmic->onforce_cmd(
				vdd_info[impu].pmic->uv_to_vsel(
					vc_config.vdd0_on));
			off_cmd = vdd_info[impu].pmic->sleepforce_cmd(
				vdd_info[impu].pmic->uv_to_vsel(
					vc_config.vdd0_off));
		...
		if (vdd_info[icore].pmic->vc_offmode_chnl_cmd != 0) {
			...
			on_cmd = vdd_info[icore].pmic->onforce_cmd(
				vdd_info[icore].pmic->uv_to_vsel(
					vc_config.vdd1_on));
			off_cmd = vdd_info[icore].pmic->sleepforce_cmd(
				vdd_info[icore].pmic->uv_to_vsel(
					vc_config.vdd1_off));
		...
		if (vdd_info[impu].pmic->vc_offmode_chnl_cmd != 0) {
			value = voltage_read_reg(
				OMAP4_PRM_VC_VAL_CMD_VDD_MPU_L_OFFSET);
			on_cmd = vc_config.vdd0_oncmd;
			off_cmd = vc_config.vdd0_offcmd;
			value = value &
				~OMAP4430_ON_MASK &
				~OMAP4430_OFF_MASK;
			voltage_write_reg(
		...
		if (vdd_info[icore].pmic->vc_offmode_chnl_cmd != 0) {
			value = voltage_read_reg(
				OMAP4_PRM_VC_VAL_CMD_VDD_CORE_L_OFFSET);
			on_cmd = vc_config.vdd1_oncmd;
			off_cmd = vc_config.vdd1_offcmd;
			value = value &
				~OMAP4430_ON_MASK &
				~OMAP4430_OFF_MASK;
			voltage_write_reg(
		...
		if (vdd_info[iiva].pmic->vc_offmode_chnl_cmd != 0) {
			value = voltage_read_reg(
				OMAP4_PRM_VC_VAL_CMD_VDD_IVA_L_OFFSET);
			on_cmd = vc_config.vdd2_oncmd;
			off_cmd = vc_config.vdd2_offcmd;
			value = value &
				~OMAP4430_ON_MASK &
				~OMAP4430_OFF_MASK;
			voltage_write_reg(
