				...
				struct omap_sr_data *sr_data)
{
	...
	pr_info("%s: sr_nvalues for %s:\n", __func__, dev_data->vdd_name);
	for (i = 0; i < dev_data->volts_supported; i++) {
		if (cpu_is_omap44xx()) {
			...
			pr_info("\ti=%d, sr_nvalue=%d, nominal_volt=%d\n",
				i, dev_data->volt_data[i].sr_nvalue,
				dev_data->volt_data[i].volt_nominal);
		} else {
