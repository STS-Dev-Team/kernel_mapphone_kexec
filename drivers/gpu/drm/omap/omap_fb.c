...
static int parse_fbinit_needed(int idx)
{
	unsigned long sizes[MAX_FBS] = {0};
	unsigned long paddrs[MAX_FBS] = {0};

	if (idx >= MAX_FBS) {
		printk(KERN_ERR"invalid fb number: %d\n", idx);
		return -EINVAL;
	}

	if (def_vram) {
		if (parse_vram_param(def_vram, MAX_FBS, sizes, paddrs)) {
			printk(KERN_ERR"failed to parse vram parameter\n");
		} else {
			if (sizes[idx] == 0)
				return -EINVAL;
		}
	...
	/* Valid non-zero positive memory size is passed so valid argument */
	return 0;
}
		...
		struct drm_mode_fb_cmd *mode_cmd)
{
	...
	if ((parse_fbinit_needed(dev->primary->index)) == -EINVAL) {
		printk(KERN_INFO"Not initialising for fb%d",
					dev->primary->index);
		goto fail;
	}
