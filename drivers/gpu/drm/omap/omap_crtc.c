...
/*Panel enable shud'nt be called in omap_connector 1st time*/
bool omap_crtc_first = true;
static int commit (struct drm_crtc *crtc)
{
	...
	/* special hack to preserve logo put up by boot-loader until we start
	 * flipping:
	 */
	if (omap_crtc_first) {
		if (crtc->x != 0 || crtc->y != 0) {
			DBG("starting normal operation");
			omap_crtc_first = false;
		} else {
			DBG("skipping");
			return 0;
		}
