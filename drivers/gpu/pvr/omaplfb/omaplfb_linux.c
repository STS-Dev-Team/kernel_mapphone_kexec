	...
	unsigned long aPhyAddr)
{
	...
	for(i = 0; i < ofbi->num_overlays ; i++)
	{
		...
		manual_update = dss_ovl_manually_updated(overlay);
		...
		if (!manual_update &&
			(manager->device->type == OMAP_DISPLAY_TYPE_HDMI))
		{
			/* Setting the overlay info for the HDMI output
			 * as the resolution may change dynamically this info
			 * needs to be reset here.
			 */
			display = manager->device;
			overlay_info.rotation_type = 0;
			overlay_info.width = display->panel.timings.x_res;
			overlay_info.height = display->panel.timings.y_res;
			overlay_info.screen_width = overlay_info.width;
			overlay_info.out_width = overlay_info.width;
			overlay_info.out_height = overlay_info.height;
			overlay_info.pos_x = 0;
			overlay_info.pos_y = 0;
			overlay_info.min_x_decim = 1;
			overlay_info.max_x_decim = 1;
			overlay_info.min_y_decim = 1;
			overlay_info.max_y_decim = 1;
			overlay_info.color_mode = OMAP_DSS_COLOR_ARGB32;
		}
