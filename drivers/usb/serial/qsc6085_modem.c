...
static int usb_serial_modem_probe(struct usb_interface *interface,
...
static int modem_dtr_control(struct usb_serial *serial, int ctrl)
{
	...
	uint16_t wIndex = modem_interface_number;
	unsigned int pipe;
	int status;
				    ...
				    unsigned int set, unsigned int clear)
{
	...
	if (modem_port_ptr->number == modem_interface_number) {
...
static void modem_interrupt_callback(struct urb *urb)
{
	...
	case -ESHUTDOWN:
		...
		return;
	case -ETIME:
		dev_err(&port->dev, "%s: Host Timed out - no response,  \n",
				__func__);
		modem_port_ptr->num_errors++;
		goto exit;
	default:
...
exit:
	spin_lock_irqsave(&modem_port_ptr->read_lock, flags);
	if (modem_port_ptr->num_errors >= 5) {
		/*
		 * If there are more than 5 timeout errors,
		 * do not submit the urb.  Instad just suspend
		 * and resume the bus to see if it recovers
		 */
		modem_port_ptr->num_errors = 0;
		if (cdma_modem_debug)
			dev_info(&modem_port_ptr->port->dev,
				"%s: Too many errors suspending the bus\n",
				__func__);
	} else if (modem_port_ptr->susp_count == 0) {
		...
		struct usb_serial_port *port)
{
	...
	for (i = 0; i < modem_port_ptr->rx_buflimit; i++) {
		list_add_tail(&(modem_port_ptr->ru[i].list),
			 &modem_port_ptr->spare_read_urbs);
	}

	for (i = 0; i < modem_port_ptr->rx_buflimit; i++) {
		list_add_tail(&(modem_port_ptr->rb[i].list),
...
static void modem_usb_disable_wakeup_irq(struct usb_interface *intf)
{
	...
	if (modem_remote_wake_gpio >= 0) {
		disable_irq_wake(gpio_to_irq(modem_remote_wake_gpio));
		disable_irq(gpio_to_irq(modem_remote_wake_gpio));
	}
}
				 ...
				 const unsigned char *buf, int count)
{
	...
	if (serial->num_bulk_out) {
		...
		if (cdma_modem_debug)
			dev_info(&port->dev, "%s: Write %d bytes.\n",
				__func__, count);
...
static int modem_usb_enable_wakeup_irq(struct usb_interface *intf)
{
	...
	if (modem_remote_wake_gpio >= 0) {
		enable_irq(gpio_to_irq(modem_remote_wake_gpio));
		enable_irq_wake(
				gpio_to_irq(modem_remote_wake_gpio));
	}
				  ...
				  const struct usb_device_id *id)
{
	...
	/* Driver have proper suspend/resume support */
	if (!ret)
...
static void modem_disconnect(struct usb_serial *serial)
{
	...
	if (modem_port_ptr->number == modem_interface_number) {
		...
		if (modem_remote_wake_gpio >= 0) {
			disable_irq_wake(gpio_to_irq(
					modem_remote_wake_gpio));
			free_irq(gpio_to_irq(modem_remote_wake_gpio),
					modem_port_ptr);
		}
