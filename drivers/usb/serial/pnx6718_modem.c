...
#define MODEM_AUTOSUSPEND_DELAY	2500
...
#define BP_STATUS_AP 53
#define AP_STATUS_BP 103
#define AP_NW  16
#define AP_NR  16
...
static int td_modem_debug = 1;
static int pnx6718_attached_ports;
static int pnx6718_wake_irq;
static bool pnx6718_wake_irq_enabled;
static DEFINE_SPINLOCK(pnx6718_wake_irq_lock);
...
struct modem_port {
	...
	struct delayed_work td_suspendfail_work;
	int irq;
...
static struct usb_device_id id_table[] = {
	{USB_DEVICE(0x04cc, 0x225c)},	/* PNX6718 BP modem */
	{},
};
...
#endif /*CONFIG_PNX6718_CTRL*/

static void td_suspendfail_workhandle(struct work_struct *work)
{
	struct delayed_work *td_suspendfail_work =
		container_of(work, struct delayed_work, work);
	struct modem_port *modem_port_ptr =
		container_of(td_suspendfail_work, struct modem_port,
							td_suspendfail_work);
	struct usb_interface *intf_to_suspend = NULL;

	if ((mdmpt != NULL) && (modem_port_ptr->port != NULL)
		&& (modem_port_ptr->port->serial)) {
		intf_to_suspend = modem_port_ptr->port->serial->interface;
		usb_autopm_get_interface(intf_to_suspend);
		usb_autopm_put_interface(intf_to_suspend);
	}
}
...
static void stop_data_traffic(struct modem_port *modem_port_ptr, int cancel)
{
	int i;
...
static int modem_tiocmget(struct tty_struct *tty)
{
...
static int modem_tiocmset(struct tty_struct *tty, unsigned int set,
					unsigned int clear)
{
	struct usb_serial_port *port = tty->driver_data;
	int status = 0;
...
static void modem_read_bulk_callback(struct urb *urb)
{
	...
	if (likely(modem_port_ptr->susp_count == 0) &&
		...
		queue_work(modem_port_ptr->sendto_tty_q,
			&modem_port_ptr->sendto_tty);
...
next_buffer:
	...
	if (tty_buffer_request_room(tty, buf->size) < buf->size) {
		pr_err("tty_buffer_request_room failed! wait for free.\n");
		spin_lock_irqsave(&modem_port_ptr->read_lock, flags);
...
static void modem_close(struct usb_serial_port *port)
{
	...
	stop_data_traffic(modem_port_ptr, 1);
	...
	if (delayed_work_pending(&modem_port_ptr->td_suspendfail_work))
		cancel_delayed_work(&modem_port_ptr->td_suspendfail_work);
...
static void modem_wake_and_write(struct work_struct *work)
{
	...
	mutex_lock(&serial->interface->dev.mutex);

	if (serial->interface->dev.power.is_suspended) {
		pr_info("[%s]: in suspend or resume, exit\n", __func__);
		mutex_unlock(&serial->interface->dev.mutex);
		spin_lock_irqsave(&modem_port_ptr->write_lock, flags);
				 ...
				 const unsigned char *buf, int count)
{
	...
	/* pr_debug("\n");*/
#endif
		spin_lock_irqsave(&pnx6718_wake_irq_lock, flags);
		if (pnx6718_wake_irq_enabled) {
			disable_irq_nosync(pnx6718_wake_irq);
			disable_irq_wake(pnx6718_wake_irq);
			pnx6718_wake_irq_enabled = false;
		}
		spin_unlock_irqrestore(&pnx6718_wake_irq_lock, flags);
		wake_lock_timeout(&modem_port_ptr->modem_wakelock,
			TIMEOUT_AFTERRX);
	...
	if (serial->num_bulk_out) {
		...
		if ((modem_port_ptr->susp_count > 0) &&
			...
			spin_unlock_irqrestore(&modem_port_ptr->write_lock,
						flags);
			pr_err("pnx6718 suspend num error!!!\n");
			return -EBUSY;
		}
...
static void modem_usb_wkup_work(struct work_struct *work)
{
	...
	if (modem_port_ptr->port == 0) {
		pr_warn("modem_port_ptr->port == 0! maybe reenum?\n");
		return;
	}
	serial = modem_port_ptr->port->serial;
	mutex_lock(&serial->interface->dev.mutex);
				   ...
				   pm_message_t message)
{
	...
	if (port->number == 0) {
		spin_lock_irq(&pnx6718_wake_irq_lock);
		if (!pnx6718_wake_irq_enabled) {
			enable_irq(pnx6718_wake_irq);
			enable_irq_wake(pnx6718_wake_irq);
			pnx6718_wake_irq_enabled = true;
		}
		spin_unlock_irq(&pnx6718_wake_irq_lock);
	}
	if (td_modem_debug)
	...
	if (!atomic_read(&td_power_off))
		stop_data_traffic(modem_port_ptr, 0);
	...
	return 0;
 err:
	schedule_delayed_work(&modem_port_ptr->td_suspendfail_work, (2000));
	return ret;
}
...
void modem_notifiy_user(void)
{
	struct usb_serial_port *serial = NULL;
	pr_info("modem_notifiy_user called\n");
	if (NULL != mdmpt) {
		serial = mdmpt->port;
		if (serial != NULL)
			kobject_uevent(&(serial->dev.kobj), KOBJ_CHANGE);
	}
}
...
static irqreturn_t bp_status_ap_irq_handler(int irq, void *data)
{
	...
	if (NULL != mdmpt) {
		...
		pr_info("%s,BP_STATUS_AP interrupt get value %d\n",
			__func__, bp_status_ap);
...
{
	if (mdmpt) {
		pr_info("[%s]: wake lock 2500ms!!\n", __func__);
		wake_lock_timeout(&mdmpt->modem_wakelock,
				TIMEOUT_AFTERRX);
	} else
		pr_info("[%s]: mdmpt is NULL, maybe disconnect\n", __func__);
}
...
static int modem_startup(struct usb_serial *serial)
{
	...
	INIT_DELAYED_WORK(&modem_port_ptr->td_suspendfail_work,
				td_suspendfail_workhandle);
	...
	if (pnx6718_attached_ports == 0) {/*ttyUSB0 for Dinara TD*/
	modem_port_ptr->irq = gpio_to_irq(BP_STATUS_AP);
	pnx6718_wake_irq = modem_port_ptr->irq;
	ret = request_irq(modem_port_ptr->irq, bp_status_ap_irq_handler,
		IRQ_TYPE_EDGE_RISING, "bp_status_ap_irq", modem_port_ptr);
	if (ret < 0) {
		pr_warn("irq bp_status_ap_irq have been requested!\n");
		modem_port_ptr->irq = 0;
	}
	mdmpt = modem_port_ptr;
	enable_irq_wake(modem_port_ptr->irq);
	disable_irq(pnx6718_wake_irq);
	}
	pr_info("pnx6718_attached_ports %d, %x ",
			pnx6718_attached_ports, (unsigned int)mdmpt);
	usb_enable_autosuspend(serial->dev);
	usb_mark_last_busy(serial->dev);

	/* the modem triggers wakeup requests only if remote wakeup set */
...
err_status_queue:
	pr_err("couldn't create usb ipc receive workqueue\n");
	return -ENOMEM;
}
...
static void modem_disconnect(struct usb_serial *serial)
{
	...
	if (delayed_work_pending(&modem_port_ptr->td_suspendfail_work))
		cancel_delayed_work_sync(&modem_port_ptr->td_suspendfail_work);
...
#endif
	if (modem_port_ptr == mdmpt)
		mdmpt = NULL;
	disable_irq_wake(pnx6718_wake_irq);
	destroy_workqueue(modem_port_ptr->sendto_tty_q);
	destroy_workqueue(modem_port_ptr->bpwk_q);
	pnx6718_wake_irq_enabled = false;
	pnx6718_attached_ports--;
}
	...
	struct usb_device *udev, char *buf, int len)
{
	...
	modem_port_ptr->wbl.buf = kmalloc(len, GFP_KERNEL);
	if (modem_port_ptr->wbl.buf < 0) {
		pr_err("[%s]: Failed to kmalloc vs_buf!\n", __func__);
		return -ENOMEM;
	}
	ret = copy_from_user(vs_ctrl, buf, len);
	if (ret < 0) {
...
#ifdef PNX_DBG`
	pr_debug("vendor string is:  ");
	for (i = 0; i < len; i++)
		pr_debug("0x%2x  ", vs_ctrl[i]);
	pr_debug("\n");
#endif
	...
	if (ret) {
		pr_err("fail to write vendor string, URB->error=%d\n", ret);
		kfree(modem_port_ptr->wbl.buf);
		usb_free_urb(urb);
		usb_autopm_put_interface(serial->interface);
		return ret;
...
static int modem_ioctl(struct tty_struct *tty, unsigned int cmd,
					unsigned long arg)
{
	struct usb_serial_port *port = tty->driver_data;
	int result = 0;
...
#ifdef PNX_DBG
	pr_info("%s - port %d, cmd %x   my is  %x %x",
		__func__, port->number, cmd, ARGETVS, ARSETVS);
#endif

	switch (cmd) {
	case ARGETVS:
		pr_info("cmd is ARGETVS\n");
		break;
	case ARSETVS:
		pr_info("cmd is ARSETVS\n");
		result = bplog_vs(modem_port_ptr, port->serial->dev,
				(char *)arg, 12);
		break;
	default:
		pr_err("No this tty ioctl cmd!!!!\n");
		return -ENOIOCTLCMD;
		break;
	}
...
int init_ipc_gpio(void)
{
	...
	if (!modem_is_ste_pnx6718())
		return 0;
	ret = gpio_request(AP_LCD_STATUS, "AP_LCD_STATUS");
	if (ret < 0) {
		pr_err("gpio_request AP_LCD_STATUS failed!\n");
		goto err3;
	}

	ret = gpio_direction_output(AP_LCD_STATUS, 0);
	if (ret < 0) {
		pr_err("gpio_direction_output AP_LCD_STATUS failed!\n");
		goto err3;
	}

	ret = gpio_request(BP_STATUS_AP, "BP Status AP");
	if (ret < 0) {
		pr_err("gpio_request BP_STATUS_AP failed!\n");
		goto err2;
	}

	ret = gpio_direction_input(BP_STATUS_AP);
	if (ret < 0) {
		pr_err("gpio_direction_input BP_STATUS_AP failed!\n");
		goto err2;
	}

	ret = gpio_request(AP_STATUS_BP, "AP Status BP");
	if (ret < 0) {
		pr_err("gpio_request AP_STATUS_BP failed!\n");
		goto err1;
	}

	ret = gpio_direction_output(AP_STATUS_BP, 0);
	if (ret < 0) {
		pr_err("gpio_direction_output AP_STATUS_BP failed!\n");
		goto err1;
	}
	return 0;
err1:
