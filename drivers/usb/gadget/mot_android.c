...
static struct device_pid_vid mot_android_vid_pid[MAX_DEVICE_TYPE_NUM] = {
	...
	{"msc_adb_eth", MSC_TYPE_FLAG | ADB_TYPE_FLAG | ETH_TYPE_FLAG,
	 ...
	 {"charge_only", MSC_TYPE_FLAG, 0x22b8, 0x42d8, "Motorola Config 49",
	 USB_CLASS_PER_INTERFACE, USB_CLASS_PER_INTERFACE,
	 USB_CLASS_PER_INTERFACE},
	{"charge_adb", MSC_TYPE_FLAG | ADB_TYPE_FLAG, 0x22b8, 0x42d9,
	 "Motorola Config 50", USB_CLASS_PER_INTERFACE,
	 USB_CLASS_PER_INTERFACE, USB_CLASS_PER_INTERFACE},
	{"rndis", RNDIS_TYPE_FLAG, 0x22b8, 0x42dc, "Motorola RNDIS Device",
	 USB_CLASS_WIRELESS_CONTROLLER, USB_CLASS_PER_INTERFACE,
	 USB_CLASS_PER_INTERFACE},
	{"rndis_adb", RNDIS_TYPE_FLAG | ADB_TYPE_FLAG, 0x22b8, 0x42dd,
	...
	{"acm_acm1_eth_mtp_adb",
	 ...
	 0x22b8,
	 0x431f, "Motorola Config 31", USB_CLASS_VENDOR_SPEC,
	 USB_CLASS_VENDOR_SPEC, USB_CLASS_VENDOR_SPEC},
	{"acm1_eth_adb",
	 ACM1_TYPE_FLAG | ETH_TYPE_FLAG | ADB_TYPE_FLAG,
	 0x22b8,
	 0x4313, "Motorola STE Config", USB_CLASS_VENDOR_SPEC,
	 USB_CLASS_VENDOR_SPEC, USB_CLASS_VENDOR_SPEC},
	{"acm_acm1_acm2_acm3_eth",
	 ACM_TYPE_FLAG | ACM1_TYPE_FLAG | ACM2_TYPE_FLAG | ACM3_TYPE_FLAG
	 | ETH_TYPE_FLAG, 0x22b8, 0x4301,
	 "Motorola Config 57", USB_CLASS_MISC,
	 0x2, 0x1},
	{"acm_acm1_acm2_acm3_eth_adb",
	 ACM_TYPE_FLAG | ACM1_TYPE_FLAG | ACM2_TYPE_FLAG | ACM3_TYPE_FLAG
	 | ETH_TYPE_FLAG | ADB_TYPE_FLAG, 0x22b8, 0x4302,
	 "Motorola Config 58", USB_CLASS_MISC,
	 0x2, 0x1},
	{"cdrom2", CDROM2_TYPE_FLAG, 0x22b8, 0x41ce, "Motorola Config 51",
	 USB_CLASS_PER_INTERFACE, USB_CLASS_PER_INTERFACE,
	 USB_CLASS_PER_INTERFACE},
	{"rndis_acm_acm1_acm2_acm3_eth",
	RNDIS_TYPE_FLAG | ACM_TYPE_FLAG | ACM1_TYPE_FLAG | ACM2_TYPE_FLAG
	| ACM3_TYPE_FLAG | ETH_TYPE_FLAG, 0x22b8, 0x4303,
	"Motorola Config 59", USB_CLASS_MISC, 0x2, 0x1},
	{"rndis_acm_acm1_acm2_acm3_eth_adb",
	RNDIS_TYPE_FLAG | ACM_TYPE_FLAG | ACM1_TYPE_FLAG | ACM2_TYPE_FLAG
	| ACM3_TYPE_FLAG | ETH_TYPE_FLAG | ADB_TYPE_FLAG, 0x22b8, 0x4304,
	"Motorola Config 60", USB_CLASS_MISC, 0x2, 0x1},
	{}
};
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	for (i = 0; i < android_config_driver.next_interface_id; i++) {
		if (android_config_driver.interface[i]->setup &&
		    !(android_config_driver.interface[i]->disabled)) {
			ret =
...
static int enable_android_usb_product_function(char *device_name, int cnt)
{
	...
	if (!strncmp(device_name, "acm1_eth_adb", cnt - 1)) {
		list_for_each_entry(f, &android_config_driver.functions,
			list) {
			if (!strcmp(f->name, "acm1")
				|| !strcmp(f->name, "usbnet")
				|| !strcmp(f->name, "adb"))
				usb_function_set_enabled(f, !disable);
			else
	...
	if (!strncmp(device_name, "msc", cnt - 1)
	    ...
	    || !strncmp(device_name, "cdrom2", cnt - 1)
			 ...
			 size_t count, loff_t *ppos)
{
	...
	int ret;
	u8 vers_info[USB_SMART_VERSION_SIZE];
	int vers_cnt = USB_SMART_VERSION_SIZE;
	static u8 vers_check;
	...
	if (!vers_check) {
		...
		if (copy_from_user(vers_info, buffer, count)) {
			printk(KERN_INFO "%s - Error copying version "
				"buffer\n", __func__);
		return -EFAULT;
		}

		vers_info[count - 1] = 0;
		if (!strncmp(vers_info, "pc_mot_drv_vers_query:", 22)) {
			memcpy(sm_vers, vers_info + 22, count - 22);
			sm_vers_sz = (count - 22);
			vers_check = 1;
			return count;
		}
