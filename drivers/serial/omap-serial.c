...
static void serial_omap_wake_peer(struct uart_port *port)
{
	...
	if (info->board_wake_peer)
		info->board_wake_peer(up);
}
			...
			struct ktermios *old)
{
	...
	if (time_after(jiffies, old_jiffies + (HZ * 5)))
		printk(KERN_ERR "omap-serial: jiffies = %ld, old_jiffies = %ld\n",
					jiffies, old_jiffies);
...
static int serial_omap_probe(struct platform_device *pdev)
{
	...
	up->console_port = omap_up_info->console_port;
...
int omap_uart_cts_wakeup(int uart_no, int state)
{
	...
	if (state) {
		...
		if ((omap_up_info->cts_padconf_mode >= OMAP_MUX_MODE0) &&
		    (omap_up_info->cts_padconf_mode <= OMAP_MUX_MODE7)) {
			if (omap_up_info->cts_padconf & 0x2) {
				...
				v |= (((OMAP_WAKEUP_EN | OMAP_OFF_PULL_EN |
					...
					omap_up_info->cts_padconf_mode)) << 16);
			} else {
				omap_up_info->cts_padvalue = 0x0000FFFF & v;
				v |= ((OMAP_WAKEUP_EN | OMAP_OFF_PULL_EN |
					OMAP_OFFOUT_VAL | OMAP_OFFOUT_EN |
					OMAP_OFF_EN | OMAP_PULL_UP |
					omap_up_info->cts_padconf_mode));
			}
		}
