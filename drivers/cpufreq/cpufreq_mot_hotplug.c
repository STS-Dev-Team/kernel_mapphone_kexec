...
#define DEFAULT_HOTPLUG_IN_LOAD				(80)

/* less than 20% avg load will hotplug-out CPU1 */
#define DEFAULT_HOTPLUG_OUT_LOAD			(20)
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_MOT_HOTPLUG
static
#endif
struct cpufreq_governor cpufreq_gov_mot_hotplug = {
       .name                   = "mot_hotplug",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	if (idle_time != -1ULL) {
		...
		pr_err("cpufreq-mot-hotplug: %s: assumes CONFIG_NO_HZ\n",
				__func__);
		return -EINVAL;
	}

	khotplug_wq = create_workqueue("kmothotplug");
	if (!khotplug_wq) {
		pr_err("Creation of kmothotplug failed\n");
		return -EFAULT;
	}
	err = cpufreq_register_governor(&cpufreq_gov_mot_hotplug);
...
{
	cpufreq_unregister_governor(&cpufreq_gov_mot_hotplug);
	destroy_workqueue(khotplug_wq);
}

MODULE_AUTHOR("Motorola")
MODULE_DESCRIPTION("'cpufreq_hotplug' - cpufreq governor for dynamic frequency \
scaling and CPU hotplugging");
MODULE_LICENSE("GPL");
