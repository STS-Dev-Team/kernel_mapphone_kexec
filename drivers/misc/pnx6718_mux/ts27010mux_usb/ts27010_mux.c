...
/* this tty is /dev/ttyUSB0, a real USB */
struct tty_struct *ts27010mux_usb_tty;
#ifdef QUEUE_SELF
struct workqueue_struct *g_mux_usb_queue;
#endif

/* there are 10 MUX USB channel and 9 MUX USB TTY device(10-18) */
...
static void ts0710_reset_dlci(struct dlci_struct *d)
{
	...
	/*
	init_waitqueue_head(&d->mux_hold_wait);
	*/
	mutex_init(&d->lock);
...
void ts27010_mux_usb_line_throttle(int line)
{
	...
	ts27010_flow_off(ts27010_usb_tty_table[line], dlci, ts0710);
}
...
int ts27010_mux_usb_line_ioctl(unsigned int cmd, unsigned long arg, int line)
{
	int dlci;
	int ret;
	struct ts0710_con *ts0710 = &ts0710_connection;
...
#ifndef MUX_USB_UT
	err = ts27010_tty_usb_open();
	if (err)
		goto err0;
	err = ts27010_usb_td_open();
	if (err)
		goto err1;
#endif
...
err1:
	ts27010_tty_usb_close();
err0:
	return err;
}

/* when USB driver disconnected but some channels are not closed,
 * set those channels to HOLD state
 */
void ts27010_mux_usb_mux_hold(void)
{
	...
	for (j = 0; j < TS0710_MAX_CHN; j++) {
		...
		if (d->state == CONNECTED)
			d->state = HOLD_CONNECTED;
		else if (d->state == FLOW_STOPPED)
			d->state = HOLD_FLOW_STOPPED;
...
void ts27010_mux_usb_mux_resume(void)
{
	...
	for (j = 0; j < TS0710_MAX_CHN; j++) {
		struct dlci_struct *d = &ts0710_connection.dlci[j];
		if (d->state == HOLD_CONNECTED) {
			d->state = CONNECTED;
			/*
			wake_up_interruptible(&d->mux_hold_wait);
			*/
		} else if (d->state == HOLD_FLOW_STOPPED) {
			d->state = FLOW_STOPPED;
			/*
			wake_up_interruptible(&d->mux_hold_wait);
			*/
	...
	for (j = 0; j < TS0710_MAX_CHN; j++) {
		...
		if (j != 0 && (d->state != DISCONNECTED || d->clients != 0)) {
			mux_print(MSG_ERROR,
				...
				j, d->state);
			/* TODO:
			 * if rild crashed, UBS line discipline will be closed
	...
	struct ts27010_ringbuf *rbuf, int start_flag)
{
	...
	if (rbuf->head > rbuf->tail) {
		mux_usb_hexdump(MSG_ERROR, "dump ringbuf",
			__func__, __LINE__,
			rbuf->buf + rbuf->tail,
			rbuf->head - rbuf->tail);
	} else {
...
#endif
#else /* TS27010_UART_RETRAN */
			/* don't do CRC check because of USB */
			if (c == TS0710_BASIC_FLAG) {
...
static int __init mux_init(void)
{
...
#ifdef QUEUE_SELF
	g_mux_usb_queue = create_workqueue("mux_usb_queue");
	if (!g_mux_usb_queue) {
	...
	err = ts27010_usb_td_init();
	if (err != 0) {
		mux_print(MSG_ERROR, "error %d td init.\n", err);
		goto err1;
	}

	mux_print(MSG_INFO, "mux over USB registered\n");

	FUNC_EXIT();
	return 0;

err1:
	ts27010_tty_usb_remove();
err0:
...
static void __exit mux_exit(void)
{
	...
	ts27010_usb_td_remove();
	ts27010_tty_usb_remove();
	ts27010_ldisc_usb_remove();

#ifdef QUEUE_SELF
	destroy_workqueue(g_mux_usb_queue);
#endif
	mux_print(MSG_INFO, "mux over USB unregistered\n");
	FUNC_EXIT();
}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("TS 07.10 Multiplexer for TD-SCDMA modem over USB");
