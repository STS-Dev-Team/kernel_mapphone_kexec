...
static void update_bp_status(void)
{
	...
	pr_debug("%s: modem status: %s -> %s [power %s]", mdmctrl,
...
static void mdm_ctrl_powerup(void)
{
	...
	pr_debug("%s: starting modem initial status %s [0x%x]",
		mdmctrl, bp_status_string(bp_status), bp_status);

	set_ap_status(AP_STATUS_NO_BYPASS);
	pr_debug("%s: ap_status set to %d", mdmctrl, get_ap_status());
	msleep(100);
	set_bp_resin(0);
	msleep(100);
