...
#define CURRENT_PRESSURE		0x55

#define CURRENT_ALTITUDE		0x57

#define ACTIVITY_DETECTION		0x6A

#define POWER_MODE			0x6B

#define NO_OF_DATA_SETS_TO_READ		0x72
				...
				struct msp430_pedometer_data *curr_data)
{
	...
	input_report_rel(ps_msp430->input_dev, REL_DIAL,
			delta_distance);
	/* report activity */
	input_report_rel(ps_msp430->input_dev, REL_MISC,
			curr_data->activity);
		...
		struct msp430_pedometer_data *pedo)
{
	...
	pedo->distance =  (msp_cmdbuff[0] << 24)
		| (msp_cmdbuff[1] << 16) | (msp_cmdbuff[2] << 8)
		| msp_cmdbuff[3];
...
static void msp430_irq_work_func(struct work_struct *work)
{
	...
	if (irq_status & M_ECOMPASS) {
		...
		xyz[1] =  (msp_cmdbuff[2] << 8) | msp_cmdbuff[3];
		xyz[2] =  (msp_cmdbuff[4] << 8) | msp_cmdbuff[5];
		input_report_abs(ps_msp430->input_dev, ABS_RX, xyz[0]);
		input_report_abs(ps_msp430->input_dev, ABS_RY, xyz[1]);
		input_report_abs(ps_msp430->input_dev, ABS_RZ, xyz[2]);
		input_sync(ps_msp430->input_dev);
		KDEBUG("Sending orientation(x,y,z)values:x = %d,y =%d,z=%d\n",
			xyz[0], xyz[1], xyz[2]);
	}
				 ...
				 size_t count, loff_t *ppos)
{
	...
	KDEBUG("MSP430 Got from user: ");
	for (i = 0; i < len; i++)
		KDEBUG("MSP430 %02x", buff[i]); /*debug */
	KDEBUG("MSP430 \n Leng = %d", len); /* debug */
...
int msp430_input_open(struct input_dev *input)
{
	...
	pr_err("MSP430 msp430_input_open\n");
...
void msp430_input_close(struct input_dev *dev)
{
	...
	pr_err("MSP430 msp430_input_close\n");
...
static int msp430_input_init(struct msp430_data *ps_msp430)
{
	...
	set_bit(ABS_VOLUME, ps_msp430->input_dev->absbit);
	...
	set_bit(ABS_HAT3X, ps_msp430->input_dev->absbit);
	set_bit(ABS_HAT3Y, ps_msp430->input_dev->absbit);
	set_bit(ABS_HAT2X, ps_msp430->input_dev->absbit);
	set_bit(ABS_HAT2Y, ps_msp430->input_dev->absbit);
