...
struct pn544_dev	{
	...
	struct mutex		read_mutex; /* mutex for read function */
		...
		const struct i2c_device_id *id)
{
	...
	if (ret) {
		/* Add comment */
		pr_err("%s : nfc probe fail, IRQ GPIO\n", __func__);
		goto err_nodev;
	}
	ret = gpio_request(platform_data->ven_gpio, "nfc_ven");
	if (ret) {
		/* Add comment */
		pr_err("%s : nfc probe fail, VEN GPIO\n", __func__);
		goto err_ven;
	}

	if (platform_data->firmware_gpio != -1) {
	...
	pr_info("PN544 Misc Minor: %d\n", pn544_dev->pn544_device.minor);
