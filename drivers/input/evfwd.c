	    ...
	    const char *buffer, size_t count)
{
	...
	if ((count % sizeof(inev)) == 0) {
		...
		for (i = 0; i < count; i += sizeof(inev)) {
			...
			switch (inev.type) {
			case EV_ABS:
				input_event(evfwd_input_dev, EV_ABS,
					inev.code, inev.value);
				break;
			case EV_REL:
				input_event(evfwd_input_dev, EV_REL,
					inev.code, inev.value);
				break;
			case EV_KEY:
				if (inev.code != KEY_UNKNOWN) {
					input_event(evfwd_input_dev, EV_KEY,
						inev.code, inev.value);
				}
				break;
			case EV_SYN:
				input_event(evfwd_input_dev,
					EV_SYN, SYN_REPORT, 0);
				break;
			default:
...
evfwd_init(void)
{
	...
	if (IS_ERR(evfwd_dev)) {
		printk(KERN_ERR "evfwd_init: error\n");
		return PTR_ERR(evfwd_dev);
	}

	/* Create a sysfs node to read simulated coordinates */
	ret = sysfs_create_group(&evfwd_dev->dev.kobj, &evfwd_attr_group);
	if (ret) {
	...
	ret = input_register_device(evfwd_input_dev);
	if (ret) {
		input_free_device(evfwd_input_dev);
...
/* Driver Exit */
void __exit evfwd_cleanup(void)
{
