...
static irqreturn_t qtouch_ts_irq_handler(int irq, void *dev_id)
{
	...
	if (ts->mode == 1)
		...
		queue_work(ts->qtouch_ts_wq, &ts->work);

	return IRQ_HANDLED;
}

static void qtouch_keyarray_ignore_timer(unsigned long handle)
{
	...
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
					...
					min(sizeof
					    ...
					    obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write user data\n", __func__);
			return ret;
		}
	}

	/* configure grip suppression 2 */
	obj = find_obj(ts, QTM_OBJ_PROCI_GRIP_SUPPRESSION2);
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
					&ts->pdata->grip_suppression2_cfg,
					min(sizeof
					    (ts->pdata->grip_suppression2_cfg),
					    obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write grip2 config\n", __func__);
			return ret;
		}
	}

	/* configure touch suppression 2 */
	obj = find_obj(ts, QTM_OBJ_PROCI_TOUCH_SUPPRESSION2);
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
				    &ts->pdata->touch_suppression2_cfg,
				    min(sizeof
				        (ts->pdata->touch_suppression2_cfg),
				        obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write touch2 config\n", __func__);
			return ret;
		}
	}

	/* configure cte config 2 */
	obj = find_obj(ts, QTM_OBJ_SPT_CTE_CONFIG2);
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
				    &ts->pdata->cte_config2_cfg,
				    min(sizeof
				        (ts->pdata->cte_config2_cfg),
				        obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write cte2 config\n", __func__);
			return ret;
		}
	}

	/* configure stylus */
	obj = find_obj(ts, QTM_OBJ_PROCI_STYLUS);
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
				    &ts->pdata->stylus_cfg,
				    min(sizeof
				        ...
				        obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write stylus config\n", __func__);
			return ret;
		}
	}

	/* configure noise suppression 2 */
	obj = find_obj(ts, QTM_OBJ_PROCG_NOISE_SUPPRESSION2);
	if (obj && obj->entry.num_inst > 0) {
		ret = qtouch_write_addr(ts, obj->entry.addr,
				    &ts->pdata->noise_suppression2_cfg,
				    min(sizeof
				        (ts->pdata->noise_suppression2_cfg),
				        obj->entry.size));
		if (ret != 0) {
			pr_err("%s: Can't write noise2 config\n", __func__);
			return ret;
		}
	}
...
static int qtouch_do_cmd_proc_msg(struct qtouch_ts_data *ts,
...
static int qtouch_do_touch_multi_msg(struct qtouch_ts_data *ts,
			   struct qtm_object *obj, void *_msg)
{
...
static int qtouch_do_touch_keyarray_msg(struct qtouch_ts_data *ts,
				 struct qtm_object *obj, void *_msg)
{
			     ...
			     struct qtm_obj_message *msg)
{
	...
	case QTM_OBJ_GEN_CMD_PROC:
		ret = qtouch_do_cmd_proc_msg(ts, obj, msg);
		break;

	case QTM_OBJ_TOUCH_MULTI:
		ret = qtouch_do_touch_multi_msg(ts, obj, msg);
		break;

	case QTM_OBJ_TOUCH_KEYARRAY:
		ret = qtouch_do_touch_keyarray_msg(ts, obj, msg);
		break;
                                 ...
                                 const char *buf, size_t size)
{
	...
	case 1:
		...
		break;
	default:
		pr_info("%s: qtouch_irq_enable failed -> irq_enabled = %d\n",
			   ...
			   const struct i2c_device_id *id)
{
	...
	ts->qtouch_ts_wq = create_singlethread_workqueue("qtouch_obp_ts_wq");
	if (ts->qtouch_ts_wq == NULL) {
	...
	if (err == 0) {
		...
		err = qtouch_read(ts, &(boot_info[0]), 3);
		if (err) {
			pr_err("%s:Read failed %d\n", __func__, err);
		} else {
			pr_info("%s:Data read 0x%x 0x%x 0x%x\n", __func__,
				boot_info[0], boot_info[1], boot_info[2]);
			loop_count = 0;
			while ((boot_info[0] & QTM_OBP_BOOT_CMD_MASK) != QTM_OBP_BOOT_WAIT_ON_BOOT_CMD) {
				err = qtouch_read(ts, &(boot_info[0]), 3);
				if (err) {
					pr_err("%s:Read failed %d\n", __func__, err);
					break;
				...
				pr_info("%s:Data read 0x%x 0x%x 0x%x\n",
					__func__, boot_info[0], boot_info[1],
					boot_info[2]);
				loop_count++;
				if (loop_count == 10) {
					err = 1;
		...
		if (!err) {
			boot_info[0] &= QTM_OBP_BOOT_VERSION_MASK;
			if (boot_info[0] & (1 << QTM_OBP_BOOT_EXTENDED_BIT))
				boot_info[0] = boot_info[1];
			pr_info("%s:Bootloader version %d\n", __func__, boot_info[0]);

			while (touch_fw_table_ptr->family_id != 0x00) {
				if (boot_info[0] == touch_fw_table_ptr->boot_version) {
					pr_info("%s: Chip type matched\n", __func__);
					ts->touch_fw_size = touch_fw_table_ptr->size;
					ts->touch_fw_image = touch_fw_table_ptr->image;
					ts->base_fw_version = touch_fw_table_ptr->base_fw_version;
	...
	if (ts->pdata->flags & QTOUCH_USE_KEYARRAY) {
		setup_timer(&keyarray_timer, qtouch_keyarray_ignore_timer, (unsigned long) ts);
	}
...
err_reading_info_block:
	...
	if (ts->qtouch_ts_wq) {
		destroy_workqueue(ts->qtouch_ts_wq);
		ts->qtouch_ts_wq = NULL;
	}
...
static int qtouch_ts_resume(struct i2c_client *client)
{
	...
	input_sync(ts->input_dev);
	/* Calling qtouch_power_config(ts, 1) looks reasonable at
	   this moment. But, touch fails to work in some condition (
