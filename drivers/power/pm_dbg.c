...
/* save the minimal suspend current drain between suspend ioctrl call */
static int min_suspend_current = 0x0fffffff;
...
static void data_update(struct pm_dbg_data *data, enum update_mode mode)
{
	...
	case IDLE_MACRO_STOP:
		...
		if (retval != 0)
			...
			if (test < min_suspend_current) {
				currmeas_data.avg_curr_susp = test;
				min_suspend_current = test;
			} else {
		      ...
		      unsigned int cmd, unsigned long arg)
{
	...
	case PM_DBG_IOCTL_CURRMEAS_SUSPEND_STS:
		if (data->suspend_meas_enable != 0) {
			...
			min_suspend_current = 0x0fffffff;
		}
