		    ...
		    loff_t *offset)
{
	...
	if (!skb) {
		...
		if (copy_to_user(data, skb->data, size))
			GPSDRV_ERR("%s: copy to user failed", __func__);
		skb_pull(skb, size);
		     ...
		     size_t size, loff_t *offset)
{
	...
	if (0 != hgps->tx_count) {
		...
		GPSDRV_VER(" SKB added to Tx queue, Tx count = %x ",
			   hgps->tx_count);
		skb_queue_tail(&hgps->tx_list, skb);
		/* This is added for precaution in the case
		 * that there is a context
		 * switch during the execution of the above lines.
		 * Redundant otherwise.
		 */
		if ((0 != hgps->tx_count)
				&& (!skb_queue_empty(&hgps->tx_list))) {
			/* Schedule the Tx-task let */
			GPSDRV_VER
			    (" Scheduling tasklet to write in gpsdrv_write");
			tasklet_schedule(&hgps->gpsdrv_tx_tsklet);
		}
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case FIONREAD:
		 ...
		 * And update the argument with skb->len to
		 * provide amount of data
