...
void if_hsi_wake_lock(int ch)
{
	struct if_hsi_channel *channel;
	channel = &hsi_iface.channels[ch];
	hsi_ioctl(channel->dev, HSI_IOCTL_WAKE_LOCK, NULL);
}

void if_hsi_wake_unlock(int ch)
{
	struct if_hsi_channel *channel;
	channel = &hsi_iface.channels[ch];
	hsi_ioctl(channel->dev, HSI_IOCTL_WAKE_UNLOCK, NULL);
}
