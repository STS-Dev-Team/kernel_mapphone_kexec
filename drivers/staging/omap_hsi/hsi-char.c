...
static struct device *hsi_char_dev;       /* HSI char dev during device_create */
...
static int __init hsi_char_init(void)
{
	...
	if (ret)
         ...
         */
        hsi_char_major = register_chrdev(0, HSI_CHAR_DEVICE_NAME, &hsi_char_fops);
        if (0 > hsi_char_major) {
                pr_debug("%s: Error when registering to char dev", __func__);
                return -EFAULT;
        }
        printk(KERN_DEBUG "%s: Allocated major device number %d", __func__, hsi_char_major);

        /*  udev */
        hsi_char_class = class_create(THIS_MODULE, HSI_CHAR_DEVICE_NAME);
        if (IS_ERR(hsi_char_class)) {
                printk(KERN_DEBUG "%s: Something went wrong in class_create", __func__);
                unregister_chrdev(hsi_char_major, HSI_CHAR_DEVICE_NAME);
                return -EFAULT;
        }

        hsi_char_dev =
          device_create(hsi_char_class, NULL, MKDEV(hsi_char_major, 0),
                          NULL, "hsi0");
        if (IS_ERR(hsi_char_dev)) {
                printk(KERN_ERR "Error in class_create");
                unregister_chrdev(hsi_char_major, HSI_CHAR_DEVICE_NAME);
...
{
        device_destroy(hsi_char_class, MKDEV(hsi_char_major, 0));
        class_destroy(hsi_char_class);
        unregister_chrdev(hsi_char_major, HSI_CHAR_DEVICE_NAME);
