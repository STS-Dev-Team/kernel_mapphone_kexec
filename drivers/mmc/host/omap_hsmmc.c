...
static void omap_hsmmc_request(struct mmc_host *mmc, struct mmc_request *req)
{
	...
	*/
	...
	if (host->id == tiwlan_mmc_controller - 1) {
...
static int omap_hsmmc_regs_show(struct seq_file *s, void *data)
{
	...
	seq_printf(s, "SYSSTATUS:\t0x%08x\n",
			OMAP_HSMMC_READ(host, SYSSTATUS));
	seq_printf(s, "CSRE:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, CSRE));
	seq_printf(s, "SYSTEST:\t0x%08x\n",
			OMAP_HSMMC_READ(host, SYSTEST));
	seq_printf(s, "CON:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, CON));
	seq_printf(s, "PWCNT:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, PWCNT));
	seq_printf(s, "BLK:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, BLK));
	seq_printf(s, "ARG:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, ARG));
	seq_printf(s, "CMD:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, CMD));
	seq_printf(s, "RSP10:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, RSP10));
	seq_printf(s, "RSP32:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, RSP32));
	seq_printf(s, "RSP54:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, RSP54));
	seq_printf(s, "RSP76:\t\t0x%08x\n",
			OMAP_HSMMC_READ(host, RSP76));
...
#ifdef CONFIG_TIWLAN_SDIO
	if (pdev->id == tiwlan_mmc_controller - 1) {
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:" MMC_DRIVER_NAME);
