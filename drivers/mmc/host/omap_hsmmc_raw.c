...
#define OMAP_HSMMC_SYSCONFIG		0x0110
#define OMAP_HSMMC_SYSSTATUS		0x0114
#define OMAP_HSMMC_CSRE		0x0124
#define OMAP_HSMMC_SYSTEST		0x0128
#define OMAP_HSMMC_CON		0x012C
#define OMAP_HSMMC_PWCNT		0x0130
#define OMAP_HSMMC_BLK		0x0204
#define OMAP_HSMMC_ARG		0x0208
#define OMAP_HSMMC_CMD		0x020C
#define OMAP_HSMMC_RSP10		0x0210
#define OMAP_HSMMC_RSP32		0x0214
#define OMAP_HSMMC_RSP54		0x0218
#define OMAP_HSMMC_RSP76		0x021C
#define OMAP_HSMMC_DATA		0x0220
#define OMAP_HSMMC_PSTATE		0x0224
#define OMAP_HSMMC_HCTL		0x0228
#define OMAP_HSMMC_SYSCTL		0x022C
#define OMAP_HSMMC_STAT		0x0230
#define OMAP_HSMMC_IE		0x0234
#define OMAP_HSMMC_ISE		0x0238
#define OMAP_HSMMC_AC12		0x023C
#define OMAP_HSMMC_CAPA		0x0240
#define OMAP_HSMMC_CUR_CAPA		0x0248

#define CM_L3INIT_CLKSTCTRL		0x4a009300
#define OMAP4_HSMMC1_ICLK		(1 << 8)
#define OMAP4_HSMMC2_ICLK		(1 << 9)
#define OMAP4_HSMMC1_FCLK		(1 << 17)
#define OMAP4_HSMMC2_FCLK		(1 << 18)

#define CM_L3INIT_HSMMC1_CLKCTRL		0x4a009328
#define CM_L3INIT_HSMMC2_CLKCTRL		0x4a009330

/* As long as in this configuration,
   power domain sleep transition can not happen. */
#define EN_MODULE_EXPLICITLY_B0		(1 << 0)
#define EN_MODULE_EXPLICITLY_B1		(1 << 1)
#endif
...
#define MMC_DRIVER_NAME		"mmci-omap-hs"
...
static void raw_omap_hsmmc_enable_clk(struct raw_omap_hsmmc_host *host)
{
	...
	switch (host->id) {
	case OMAP_MMC1_DEVID:
		reg = omap_readl(CM_L3INIT_HSMMC1_CLKCTRL);
		reg &= EN_MODULE_EXPLICITLY_B0;
		reg |= EN_MODULE_EXPLICITLY_B1;
		omap_writel(reg, CM_L3INIT_HSMMC1_CLKCTRL);
		break;
	case OMAP_MMC2_DEVID:
		reg = omap_readl(CM_L3INIT_HSMMC2_CLKCTRL);
		reg &= EN_MODULE_EXPLICITLY_B0;
		reg |= EN_MODULE_EXPLICITLY_B1;
		omap_writel(reg, CM_L3INIT_HSMMC2_CLKCTRL);
		break;
	default:
		break;
	}
...
#elif defined(CONFIG_ARCH_OMAP4)
	reg = omap_readl(CM_L3INIT_CLKSTCTRL);
	reg |= OMAP4_HSMMC1_ICLK | OMAP4_HSMMC2_ICLK
		 | OMAP4_HSMMC1_FCLK | OMAP4_HSMMC2_FCLK;
	omap_writel(reg, CM_L3INIT_CLKSTCTRL);
#endif
...
#define SECTOR_NUM_FOR_2G_DENSITY (2*1024*1024*2)
static int raw_mmc_read_ext_csd(struct raw_omap_hsmmc_host *host)
{
	...
	/* default to block addressed eMMC */
	host->card.state |= MMC_STATE_BLOCKADDR;
	...
	if (raw_ext_csd[EXT_CSD_REV] > 5) {
		printk(KERN_ERR "KPANIC-MMC: unrecognised EXT_CSD structure"
			" version %d, ignoring.\n", raw_ext_csd[EXT_CSD_REV]);
		err = -1;
		goto out;
	}

	sectors = raw_ext_csd[EXT_CSD_SEC_CNT + 0] << 0 |
		raw_ext_csd[EXT_CSD_SEC_CNT + 1] << 8 |
		raw_ext_csd[EXT_CSD_SEC_CNT + 2] << 16 |
		raw_ext_csd[EXT_CSD_SEC_CNT + 3] << 24;
	/* Cards with density > 2GiB are sector addressed */
	if (sectors <= SECTOR_NUM_FOR_2G_DENSITY)
		host->card.state &= ~MMC_STATE_BLOCKADDR;
