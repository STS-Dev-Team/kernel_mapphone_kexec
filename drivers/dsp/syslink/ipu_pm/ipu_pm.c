				       ...
				       struct ipu_pm_params *params)
{
	...
	cam_reg_prev_volt = regulator_get_voltage(p_regulator);

	if ((cam_reg_prev_volt < rcb_p->data[0]) ||
	    (cam_reg_prev_volt > rcb_p->data[1])) {
	...
	retval = regulator_is_enabled(p_regulator);
	if (retval < 0)
		goto error;
				     ...
				     struct ipu_pm_params *params)
{
	...
	if (AUX_CLK_USE_MASK & (1 << pm_aux_clk_num)) {
		...
		MASK_SET_FIELD(a_clk, AUX_CLK_CLKDIV, 0x7);
				       ...
				       struct ipu_pm_params *params)
{
	...
	cur_voltage = regulator_get_voltage(p_regulator);
	if (cur_voltage != cam_reg_prev_volt) {
		/* Restart the voltage to the default value */
		retval = regulator_set_voltage(p_regulator, cam_reg_prev_volt,
				cam_reg_prev_volt);
	}
	...
	if (retval < 0) {
		pr_err("%s %d Error disable regulator voltage\n",
			__func__, __LINE__);
		return PM_INVAL_REGULATOR;
	}
