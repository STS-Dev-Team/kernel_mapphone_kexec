...
// HACK to reuse GFX Buffers

unsigned long gfx_memory_paddr[VIDEO_MAX_FRAME];
EXPORT_SYMBOL(gfx_memory_paddr);

struct mutex wb_lock;
static struct videobuf_queue_ops video_vbq_ops;

static int debug_wb = 0;
...
int omap_setup_wb(struct omap_wb_device *wb_device, u32 addr, u32 uv_addr)
{
	...
	if (wb_info.source_type == V4L2_WB_SOURCE_OVERLAY) {
		if ((ovl = omap_dss_get_overlay(wb_info.source - 3)) == NULL) {
			printk(KERN_ERR WB_NAME
					"invalid WB source overlay :%d\n",
					wb_info.source);
			return -EINVAL;
		}
		ovl->get_overlay_info(ovl, &info);

		/* setup to capture out win size of pipeline */
		/* wb_info.height = info.out_height;
		 * wb_info.width =  info.out_width;
		 */
	...
	for (i = 0; i < omap_dss_get_num_overlay_managers(); ++i) {
			...
			if (wb_device->source == V4L2_WB_LCD_1_MANAGER) {
				if (strcmp(mgr->name, "lcd") == 0)
					break;
			} else if (wb_device->source == V4L2_WB_LCD_2_MANAGER) {
				if (strcmp(mgr->name, "2lcd") == 0)
					break;
			}
	}
...
static int omap_wb_process_frame(struct omap_wb_device *wb)
{
	int ret = 0;
	u32 addr, uv_addr;
	wb->cur_frm = list_entry(wb->dma_queue.next,
...
static int omap_wb_get_irq_mask(struct omap_wb_device *wb)
{
	int mask;

	mask = DISPC_IRQ_FRAMEDONE_WB;
	switch (wb->source) {
		case V4L2_WB_OVERLAY0:
		case V4L2_WB_OVERLAY1:
		case V4L2_WB_OVERLAY2:
			mask = DISPC_IRQ_FRAMEDONE_WB;
			return mask;
		case  V4L2_WB_LCD_1_MANAGER:
			mask |= DISPC_IRQ_FRAMEDONE | DISPC_IRQ_VSYNC;
			return mask;
		case V4L2_WB_LCD_2_MANAGER:
			mask |= DISPC_IRQ_FRAMEDONE2 | DISPC_IRQ_VSYNC2;
			return mask;
		case V4L2_WB_TV_MANAGER:
			mask |=	 DISPC_IRQ_EVSYNC_EVEN | DISPC_IRQ_EVSYNC_ODD;
			return mask;
		default:
			v4l2_dbg(1, debug_wb, &wb->wb_dev->v4l2_dev,
					"error:invalid wb source\n");
			return -EINVAL;
	}
}
			     ...
			     int cmd, void *arg)
{
	...
	struct omap_wb_device *wb = fh;
	struct omap_writeback *writeback;
	int i;
	...
	case VIDIOC_CUSTOM_G_WB:
		...
		break;
	case VIDIOC_CUSTOM_TRIGGER_WB:
		writeback = omap_dss_get_wb(0);
		if (writeback) {
			writeback->triggered = true;
			/* Entry Point to call configure_dispc() */
			mgr = NULL;
			for (i = 0; i < omap_dss_get_num_overlay_managers();
			     ++i) {
				mgr = omap_dss_get_overlay_manager(i);
				if (mgr && wb->source == V4L2_WB_LCD_1_MANAGER)
					if (strcmp(mgr->name, "lcd") == 0)
						break;
				if (mgr && wb->source == V4L2_WB_LCD_2_MANAGER)
					if (strcmp(mgr->name, "2lcd") == 0)
						break;
			}
			if (mgr)
					...
					struct v4l2_pix_format *pix)
{
	...
	for (i = start; i < start + n_alloc; i++) {
		/* HACK to share memory with GFX V4L2 driver */
		wb->buf_phy_addr[i] = gfx_memory_paddr[i];
...
static int omap_wb_open(struct file *file)
{
	...
	if (writeback) {
		writeback->triggered = false;
		writeback->in_progress = false;
		writeback->buffer_available = false;
	}
...
void omap_wb_isr(void *arg, unsigned int irqstatus)
{
	...
	if (list_empty(&wb->dma_queue)) {
		/* No more buffers so WB pipe is no longer in progress */
		if (writeback)
			writeback->in_progress = false;
		spin_unlock_irqrestore(&wb->vbq_lock, flags);
		/* There are no more available buffers to do Writeback
		   so Writeback is done and we don't need to trigger
		   anymore, so it is safe to disable the DSS clocks.
		   The trigger for this WB is in configure_dispc() in
	...
	if (writeback) {
		/* Activate Writeback if Triggered */
		if (writeback->triggered) {
			/* No longer is buffer available */
			writeback->buffer_available = false;
			/* Clear Trigger Flag */
			writeback->triggered = false;
			/* No Need to enable DSS clocks here, because
			   on entry to this ISR, the clocks had to
			   be enabled to trigger this WB.  Retrigger
			   writeback, and clocks will be disabled when
			   there are no outstanding transactions */
			dispc_go_wb();
			dispc_enable_plane(OMAP_DSS_WB, 1);
		} else {
			/* Buffer was available, but no triggered update so
			   WB pipe is no longer in progress */
			writeback->in_progress = false;
			/* No triggered WB to perform, so no writeback
			   will be in progress.  It is safe now to turn
			   off DSS clocks.  The corresponding enable
			   was triggered in configure_dispc() in
			   drivers/video/omap2/dss/manager.c */
			dss_mainclk_disable();
		}
        }
