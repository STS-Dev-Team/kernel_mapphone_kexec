...
/* Manual update work queue depth (must be power of 2)*/
#define Q_DEPTH                 (1<<3)
...
#ifdef CONFIG_OMAP2_DSS_MANAGER
	if (wb_on || ((manually_updated(vout)) ||
                       (cur_display->type == OMAP_DISPLAY_TYPE_HDMI))) {
#else
	if (wb_on || manually_updated(vout)) {
#endif
		if (w && atomic_read(&vout->work_items) < Q_DEPTH) {
			w[vout->next_work].vout = vout;
			w[vout->next_work].process = process;
			queue_work(vout->workqueue,
				   &(w[vout->next_work].work));
			vout->next_work = (vout->next_work + 1) & (Q_DEPTH-1);
			atomic_inc(&vout->work_items);
		} else
			printk(KERN_ERR "No available work slot, "
					"dropping frame\n");
	} else {
...
#ifdef CONFIG_PM
	if (pdata->set_min_mpu_freq)
		pdata->set_min_mpu_freq(((vout->vid_dev)->v4l2_dev).dev, -1);
#endif
			...
			struct v4l2_format *f)
{
	...
	if (!ovl->manager)
		...
		if (wb) {
			vout->fbuf.fmt.height = wb->info.height;
			vout->fbuf.fmt.width = wb->info.width;
		} else {
			vout->fbuf.fmt.height = QQVGA_HEIGHT;
			vout->fbuf.fmt.width = QQVGA_WIDTH;
		}
        }
...
static int vidioc_s_crop(struct file *file, void *fh, struct v4l2_crop *crop)
{
	...
	if (!ovl->manager) {
		...
		if (wb) {
			if (rotate_90_or_270(vout)) {
				vout->fbuf.fmt.height = wb->info.width;
				vout->fbuf.fmt.width = wb->info.height;
			} else {
				vout->fbuf.fmt.height = wb->info.height;
				vout->fbuf.fmt.width = wb->info.width;
			}
			...
			if (rotate_90_or_270(vout)) {
				vout->fbuf.fmt.height = QQVGA_WIDTH;
				vout->fbuf.fmt.width = QQVGA_HEIGHT;
			} else {
				vout->fbuf.fmt.height = QQVGA_HEIGHT;
				vout->fbuf.fmt.width = QQVGA_WIDTH;
			}
	...
	if (crop->type == V4L2_BUF_TYPE_VIDEO_OUTPUT)
		ret = omap_vout_new_crop(&vout->pix, &vout->crop, &vout->win,
				&vout->fbuf, &crop->c, rotate_90_or_270(vout));
...
static int vidioc_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	case V4L2_CID_PRIVATE_TAKE_CONSTRAINT:
	{
#ifdef CONFIG_PM
		int constraint = a->value;
		struct vout_platform_data *pdata =
			(((vout->vid_dev)->v4l2_dev).dev)->platform_data;
		if (constraint) {
			if (pdata->set_min_mpu_freq) {
					pdata->set_min_mpu_freq(
					((vout->vid_dev)->v4l2_dev).dev ,
						600000000);
			}
		} else {
			...
			struct v4l2_buffer *buffer)
{
	...
	if (((wb && wb->enabled) || vout->wb_enabled ||
	     manually_updated(vout)) &&
	    !vout->first_int &&
	    vout->cur_frm &&
	    vout->next_frm &&
...
#ifndef CONFIG_ARCH_OMAP4
	/*
	 * Depending on what we do with the video buffer, we might need to
	 * use the VRFB.  So having more video buffers than VRFB buffers is
	 * not useful (would have to hold onto/not display a video buffer when
	 * a VRFB buffer is not available.
	 */
...
static int __init omap_vout_create_video_devices(struct platform_device *pdev)
{
	...
	for (k = 0; k < pdev->num_resources; k++) {
		...
		vout->work = kzalloc(sizeof(struct omap_vout_work) * Q_DEPTH,
					GFP_KERNEL);
		if (vout->work == NULL) {
			ret = -ENOMEM;
			goto error_q;
		...
		for (l = 0; l < Q_DEPTH; l++) {
			INIT_WORK(&(vout->work[l].work),
				  omapvid_process_frame_work);
		}
		atomic_set(&vout->work_items, 0);
		vout->next_work = 0;
#endif
	...
	for (i = 1; i < vid_dev->num_overlays; i++) {
		...
		if (def_display) {
			...
			if (def_display->caps &
				...
				if (machine_is_mapphone() && dssdrv->support_te)
					sup_te =
						dssdrv->support_te(def_display);
#ifdef CONFIG_FB_OMAP2_FORCE_AUTO_UPDATE
				if (dssdrv->enable_te)
					dssdrv->enable_te(def_display, sup_te);
