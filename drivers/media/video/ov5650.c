...
static int ov5650_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		if (ov5650->pdata->pre_poweron) {
			ret = ov5650->pdata->pre_poweron(sd);
			if (ret) {
				dev_err(dev,
		...
		if (ov5650->dovdd) {
			ret = regulator_enable(ov5650->dovdd);
			if (ret) {
				dev_err(dev,
					"Error in enabling DOVDD (%d)\n", ret);
				if (ov5650->pdata->post_poweroff)
					ov5650->pdata->post_poweroff(sd);
				return ret;
			}
		}

		if (ov5650->avdd) {
			ret = regulator_enable(ov5650->avdd);
			if (ret) {
				dev_err(dev,
					"Error in enabling AVDD (%d)\n", ret);
				if (ov5650->dovdd)
					regulator_disable(ov5650->dovdd);
				if (ov5650->pdata->post_poweroff)
					ov5650->pdata->post_poweroff(sd);
				return ret;
			}
		...
		ret = clk_enable(ov5650->xvclk);
		if (ret) {
			dev_err(dev, "Error in enabling XVCLK (%d)\n", ret);
			if (ov5650->avdd)
				regulator_disable(ov5650->avdd);
			if (ov5650->dovdd)
				regulator_disable(ov5650->dovdd);
			if (ov5650->pdata->post_poweroff)
				ov5650->pdata->post_poweroff(sd);
		...
		if (gpio_is_valid(ov5650->pdata->gpio_pwdn)) {
			gpio_set_value(ov5650->pdata->gpio_pwdn,
				       ov5650->pdata->is_gpio_pwdn_acthi ?
				       1 : 0);
		}
...
static int ov5650_get_resources(struct ov5650 *ov5650, struct device *dev)
{
	const struct ov5650_platform_data *pdata = ov5650->pdata;
	int ret = 0;

	ov5650->xvclk = clk_get(dev, pdata->clk_xvclk);
	if (IS_ERR(ov5650->xvclk)) {
		dev_err(dev, "Unable to get XVCLK (%s)\n", pdata->clk_xvclk);
		return -ENODEV;
	}

	if (clk_round_rate(ov5650->xvclk, 24000000) != 24000000)
		dev_warn(dev, "XVCLK set to rounded aproximate (%lu Hz)",
			 clk_round_rate(ov5650->xvclk, 24000000));

	if (clk_set_rate(ov5650->xvclk,
			 clk_round_rate(ov5650->xvclk, 24000000))) {
	...
	ov5650->avdd = regulator_get(dev, pdata->reg_avdd);
	if (IS_ERR(ov5650->avdd)) {
	...
	if (regulator_set_voltage(ov5650->avdd, 2800000, 2800000)) {
		dev_err(dev, "Unable to set valid AVDD (%s) regulator"
			" voltage to: 2.8V\n", pdata->reg_avdd);
		ret = -ENODEV;
		regulator_put(ov5650->avdd);
		goto err_reg_avdd;
	}
...
get_reg_dovdd:
	...
	ov5650->dovdd = regulator_get(dev, pdata->reg_dovdd);
	if (IS_ERR(ov5650->dovdd)) {
	...
	if (regulator_set_voltage(ov5650->dovdd, 1800000, 1800000)) {
		dev_err(dev, "Unable to set valid DOVDD (%s) regulator"
			" voltage to: 1.8V\n", pdata->reg_dovdd);
		ret = -ENODEV;
		regulator_put(ov5650->dovdd);
		goto err_reg_dovdd;
	}
...
err_reg_avdd:
err_clk_set_rate:
	clk_put(ov5650->xvclk);

	return ret;
}

static void ov5650_put_resources(struct ov5650 *ov5650)
{
	if (gpio_is_valid(ov5650->pdata->gpio_resetb))
		gpio_free(ov5650->pdata->gpio_resetb);
	if (gpio_is_valid(ov5650->pdata->gpio_pwdn))
		gpio_free(ov5650->pdata->gpio_pwdn);
	regulator_put(ov5650->dovdd);
	regulator_put(ov5650->avdd);
	clk_put(ov5650->xvclk);
}
			 ...
			 const struct i2c_device_id *did)
{
	...
	ret = ov5650_get_resources(ov5650, &client->dev);
	if (ret) {
		kfree(ov5650);
		return ret;
	}
