		...
		unsigned long paddr)
{
	...
	if (!size)
#ifdef CONFIG_SGX540
		/* pvr drivers require double buffered fb for HW drivers*/
...
static int get_num_fbs(struct omapfb_platform_data *opd)
{
	DBG("Feature fb1 to vid2 = %d\n", opd->feature_fb1_to_vid2);

	if (opd->feature_fb1_to_vid2 && opd->num_fbs)
		return opd->num_fbs;

	return CONFIG_FB_OMAP2_NUM_FBS;
}

static int omapfb_create_framebuffers(struct omapfb2_device *fbdev)
{
	int r, i, total_framebuffers = 0;
	struct omapfb_platform_data *opd;
	...
	/* allocate fb_infos */
	opd = fbdev->dev->platform_data;
	total_framebuffers = get_num_fbs(opd);

	DBG("create %d framebuffers\n",	total_framebuffers);

	for (i = 0; i < total_framebuffers; i++) {
	...
	for (i = 0; i < min(fbdev->num_fbs, fbdev->num_overlays); i++) {
		...
		/* Set fb1's overlay to overlay2 instead of overlay1 */
		if (i == 1 && fbdev->num_overlays >= 3 && opd->feature_fb1_to_vid2)
			ofbi->overlays[0] = fbdev->overlays[2];
		else
			ofbi->overlays[0] = fbdev->overlays[i];

		DBG("Setting the overlay %d for fb %d\n", ofbi->overlays[0]->id, i);
		ofbi->num_overlays = 1;
	}
	...
	if (def_display) {
		...
		if (def_display->caps & OMAP_DSS_DISPLAY_CAP_MANUAL_UPDATE) {
			...
			if (machine_is_mapphone() && dssdrv->support_te)
				sup_te = dssdrv->support_te(def_display);
