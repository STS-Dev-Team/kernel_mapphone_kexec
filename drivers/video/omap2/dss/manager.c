...
#define ACTUAL_DSS_MANAGERS (cpu_is_omap44xx() ? MAX_DSS_MANAGERS : 2)
...
static int configure_overlay(enum omap_plane plane)
{
	...
	if (cpu_is_omap44xx()) {
		...
		if (wb->enabled && omap_dss_check_wb(wb, plane, -1))
			source_of_wb = true;
	}
...
static int configure_dispc(void)
{
	...
	const int num_mgrs = ACTUAL_DSS_MANAGERS;
	int i;
	int r;
	...
	for (i = 0; i < num_ovls; ++i) {
		...
		if (wb->enabled &&
		    ((oc->channel == OMAP_DSS_CHANNEL_LCD &&
		      wb->source == OMAP_WB_LCD_1_MANAGER) ||
		     (oc->channel == OMAP_DSS_CHANNEL_LCD2 &&
		      wb->source == OMAP_WB_LCD_2_MANAGER))) {
			/* If this overlay is associated with an enabled
			   writeback source, we need to trigger the WB */
	...
	if (wb->enabled && wb->source_type == OMAP_WB_SOURCE_MANAGER &&
			wb->dirty) {
		/* writeback is enabled for this plane - set accordingly */
		dispc_setup_wb(wb);
		wb->dirty = false;
		wb->shadow_dirty = true;
		writeback->buffer_available = true;
	}
	...
	/* Process triggers for WB module */
	if (writeback->enabled &&
	    !writeback->in_progress &&
	    writeback->buffer_available &&
	    writeback->triggered) {
		/* If the module is enabled, no WB is in progress, buffer is
		   available and a source has triggered, than start WB pipe */

		/* No longer is buffer available */
		writeback->buffer_available = false;
		/* Clear Trigger Flag */
		writeback->triggered = false;
		writeback->in_progress = true;
		/* This enable is to ensure that the DSS clocks are on when
		   a Writeback is Triggered.	Triggering of a WB here will
		   result in a DISPC_IRQ_FRAMEDONE_WB interrupt on completion.
		   It is in this interrupt (omap_wb_isr) that we have the
		   matching dss_mainclk_disable() call.
		   (See drivers/media/video/omap/omap_wb.c) */
		dss_mainclk_enable();
		dispc_go_wb();
		dispc_enable_plane(OMAP_DSS_WB, 1);
	}
...
static int omap_dss_mgr_apply(struct omap_overlay_manager *mgr)
{
	...
	/*
	  * FixMe: This is to work around BUG in mutex.c when
	  * calling pm_runtime API between spin_lock_irqsave()
	...
	for (i = 0; i < omap_dss_get_num_overlays(); ++i) {
		...
		if (cpu_is_omap44xx() && wbc->enabled &&
			...
			continue;
                ...
                // Display Doesn't Need to Exist for Writeback
...
{
	struct overlay_cache_data *oc = NULL;
	struct omap_overlay *ovl;
	struct writeback_cache_data *wbc;
	unsigned long flags;
	int i;
	...
	if (wb && wb->enabled) {
		...
		if (wb->info.source_type == OMAP_WB_SOURCE_MANAGER)
			wbc->input_color_mode = wbc->color_mode;
		else
...
int dss_init_overlay_managers(struct platform_device *pdev)
{
	...
	for (i = 0; i < ACTUAL_DSS_MANAGERS; ++i) {
		struct omap_overlay_manager *mgr;
