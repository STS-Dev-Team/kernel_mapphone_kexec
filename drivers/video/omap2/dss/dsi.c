...
#define DSI_DT_RX_GENERIC_LONG_READ	0x1a
...
static struct dsi_struct
{
	...
	struct {
		enum omap_dsi_index_vc_mode mode;
		struct omap_dss_device *dssdev;
...
enum omap_dsi_index_pll_power_state {
...
static int dsi_pll_power(enum omap_dsi_index ix,
	enum omap_dsi_index_pll_power_state state)
{
	int t = 0;
...
{
	int r = 0;
	enum omap_dsi_index_pll_power_state pwstate;
	enum omap_dsi_index ix;
...
static int dsi_complexio_init(struct omap_dss_device *dssdev)
{
	...
	if (cpu_is_omap44xx()) {
		...
		REG_FLD_MOD(ix, DSI_CLK_CTRL, 0, 13, 13);
...
void dsi_disable_vid_vc_enable_cmd_vc(enum omap_dsi_index lcd_ix)
{
...
void dsi_disable_cmd_vc_enable_vid_vc(enum omap_dsi_index lcd_ix)
{
	dsi_if_enable(lcd_ix, 0);
	dsi_vc_enable(lcd_ix, 1, 0);
	omapdss_dsi_vc_enable_hs(lcd_ix, 0, 1);
	dsi_if_enable(lcd_ix, 1);
	dsi_force_tx_stop_mode_io(lcd_ix);
}
		...
		u8 ecc)
{
	...
	/*
	 * This change is part of workaournd in mapphone_set_update_window()
	 * sending page/column cmd using dsi_vc_dcs_write() with BTA sync.
	 * with this delay, iclk can be gated 100% without failure when frame
	 * done.
	 */
	udelay(1);
...
int dsi_vc_write_nosync(enum omap_dsi_index lcd_ix, int channel,
			u8 data_type, u8 *data, int len)
{
	...
	if (len == 1)
		r = dsi_vc_send_short(lcd_ix, channel, data_type, data[0], 0);
	else if (len == 2)
		r = dsi_vc_send_short(lcd_ix, channel, data_type,
						data[0] | (data[1] << 8), 0);
	else
		r = dsi_vc_send_long(lcd_ix, channel, data_type, data, len, 0);

	return r;
}
EXPORT_SYMBOL(dsi_vc_write_nosync);

int dsi_vc_write(enum omap_dsi_index lcd_ix, int channel, u8 data_type, u8 *data, int len)
{
	int r;

	r = dsi_vc_write_nosync(lcd_ix, channel, data_type, data, len);
	if (r)
		return r;
	...
	u8 dcs_cmd, u8 *buf, int buflen)
{
	...
	if (dt == DSI_DT_RX_ACK_WITH_ERR) {
		...
		(dt == DSI_DT_RX_GENERIC_LONG_READ)) {
		int w;
		...
		u16 x, u16 y, u16 w, u16 h)
{
	...
	if ((p_dsi->te_enabled) &&
		(p_dsi->te_type == OMAP_DSI_TE_CMOS_TE_0)) {
		dsi_write_reg(ix, DSI_TE_HSYNC_NUMBER(ix), 0x00000000);
		dsi_write_reg(ix, DSI_TE_VSYNC_WIDTH(ix), 0x00000100);
	...
	if ((p_dsi->te_enabled) && (p_dsi->te_type == OMAP_DSI_TE_MIPI_PHY)) {
	 ...
	 * 0xf810405c when Framedone not received for 250ms! LR is at
	 * dss_mainclk_disable/dss_opt_clock_disable.
	 *
	 ...
	 */
	hrtimer_start(&p_dsi->fd_guard_timer,
...
static void dsi_handle_framedone(enum omap_dsi_index ix, int error)
{
	...
	/* Relealse DSS clks when frame done */
	dss_mainclk_disable();
	dsi_bus_unlock(ix);
}
					...
					(struct hrtimer *fd_guard_timer)
{
	...
	dsi1.fd_timeout_flag = true;

	if (dsi1.fd_flag == false) {
...
static enum hrtimer_restart dsi2_framedone_timeout_hrtimer_cb
					(struct hrtimer *fd_guard_timer)
{
	/*
	 * Sometimes, depending on the framedone guard timer setting and DSS
	...
	dsi2.fd_timeout_flag = true;

	if (dsi2.fd_flag == false) {
		DSSERR("Framedone not received for 250ms!\n");
...
static void dsi_framedone_irq_callback(void *data, u32 mask)
{
	...
	p_dsi->fd_flag = true;
	hrtimer_cancel(&p_dsi->fd_guard_timer);
	...
	r = dss_mainclk_enable();
	if (r) {
		printk(KERN_ERR"%s: enable dss clk failed\n", __func__);
		return;
	}
	...
	if (p_dsi->fd_timeout_flag == true) {
		printk(KERN_INFO " Received FD IRQ, but timeout might have"
				" happened then ignore the timeout message \n");
		/*
...
static void dsi2_framedone_irq_callback(void *data, u32 mask)
{
	...
	if (p_dsi->fd_timeout_flag == true) {
		printk(KERN_INFO " Received FD IRQ, but timeout migh has"
				" happened then ignore the timeout message \n");
		/*
		 * Because farmedone guard timeout was happened and it had
		 * cleaning up and unlock the dsi bus, therefore we should do
		 * nothing
		 */
		goto end;
...
void reconfigure_dsi_pll(struct omap_dss_device *dssdev)
{
	...
	/* FIXME... This is  a function of the panel file therefore this
	 * must be moved to the panel file..
	 * few qHD panel ES1 can not run at fast clock,
	 * Hard code ES1 mipi dsi dpll clk configure here
	 * to overwrite devtree configuration.
	 */
	DSSDBG("reconfigure_dsi_pll.\n");

	dssdev->phy.dsi.div.regn = 13;
	dssdev->phy.dsi.div.regm = 170;
	dssdev->phy.dsi.div.regm_dispc = 5;
	dssdev->phy.dsi.div.regm_dsi = 5;
	dssdev->phy.dsi.div.lp_clk_div = 8;
	dssdev->phy.dsi.div.lck_div = 1;
	dssdev->phy.dsi.div.pck_div = 7;
...
int omapdss_dsi_display_enable(struct omap_dss_device *dssdev)
{
	...
	r = dss_mainclk_state_enable();
	if (r) {
		DSSERR("Fail to call dss_mainclk_state_enable. ret=0x%x\n", r);
		goto err;
	}
...
void dsi_cleanup_irq(enum omap_dsi_index ix)
{
	dsi_write_reg(ix, DSI_IRQENABLE, 0x0);
	dsi_write_reg(ix, DSI_IRQSTATUS, 0xffffffff);

	dsi_write_reg(ix, DSI_COMPLEXIO_IRQ_ENABLE, 0x0);
	dsi_write_reg(ix, DSI_COMPLEXIO_IRQ_STATUS, 0xffffffff);
}
...
int dsi_init(struct platform_device *pdev)
{
	...
	hrtimer_init(&dsi1.fd_guard_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	dsi1.fd_guard_timer.function = dsi_framedone_timeout_hrtimer_cb;
	...
	dsi1_mem = platform_get_resource(pdev, IORESOURCE_MEM, 1);
        if (dsi1_mem == NULL) {
                DSSERR("failed to call platform_get_resource() \n");
                r = -ENXIO;
                goto err1;
        }
	...
	/*
	 * Need to disable DSI/VC/COMPLEXIO_IRQENABLE
	 * and clear DSI/VC/COMPLEXIO_IRQSTATUS here, so no spurious irqs
	 * are deliverd before the dsi block is fully initialzed --
	 * this is especially an issue if bootloader initialized DSS.
	 */
	dsi_cleanup_irq(ix);
...
int dsi2_init(struct platform_device *pdev)
{
	...
	hrtimer_init(&dsi2.fd_guard_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	dsi2.fd_guard_timer.function = dsi2_framedone_timeout_hrtimer_cb;
	...
	dsi2.te_type = OMAP_DSI_TE_MIPI_PHY;
