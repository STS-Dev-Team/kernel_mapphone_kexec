...
static int dss_get_clocks(void)
{
	...
	if (cpu_is_omap44xx()) {
		r = dss_get_clock(&core.dss_fck, "dss_fck");
		if (r)
			goto err;
...
int dss_mainclk_enable()
{
	...
	if (cpu_is_omap44xx()) {
		r = dss_opt_clock_enable();
		if (!r) {
			r = clk_enable(core.dss_fck);
			if (r) {
				dss_opt_clock_disable();
				DSSERR("%s:failed to enable core.dss_fck,"
					" ret = %d.\n", __func__, r);
			}
		} else
			DSSWARN("%s: dss_opt_clock_enable ret = %d.\n",
				 __func__, r);
	}
...
static int dss_driver_probe(struct device *dev)
{
	...
	DSSDBG("\ndriver_probe: %s: device name: %s", dssdev->driver_name, dev_name(dev));

	if (pdata->ovl2mgr_mapping_cnt &&
		(dssdev->type == OMAP_DISPLAY_TYPE_HDMI)) {
		/* In the above function the overlays get attached to the
		 * default ovl mgr but we need to override the connections
		 * with respect to platform configuration data.
		 */
		DSSDBG("\nReconnecting the overlay to the specified device manager\n");
		dss_read_ovl2mgr_platform_cfg(dssdev, pdata->ovl2mgr_mapping,
						pdata->ovl2mgr_mapping_cnt);
	}
