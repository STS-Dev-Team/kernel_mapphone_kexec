...
static int omap_dss_unset_manager(struct omap_overlay *ovl)
{
	...
	if (r) {
		/* Ignore timeout case for HDMI devices since HDMI devices will
		 * set the device state to active even while no data is being
		 * output.  This is due to the requirement of enabling the HDMI
		 * clocks, PLL, etc. in order to read the EDID, which must be
		 * done prior to enabling the output.  The state is set to
		 * active to allow for proper suspend/resume handling.
		 */
		if (r != -ETIMEDOUT || ovl->manager->device == NULL ||
		    ovl->manager->device->type != OMAP_DISPLAY_TYPE_HDMI)
			return r;
	}
...
/* Description: This function reconnects the right overlays to the manager with
 * respect to platform configuration data
 */
