		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	code = val / 10;
	mode = val % 10;
		if (!dssdev->driver->set_edid_timing)
			return -ENOENT;
	dssdev->driver->set_edid_timing(dssdev, mode, code);
		...
		const char *buf, size_t size)
{
	...
	if (enabled != (dssdev->state != OMAP_DSS_DISPLAY_DISABLED)) {
		r = dssdev->driver->hpd_enable(dssdev, enabled);
		if (r)
			return r;
	}
...
int omapdss_default_check_timings(struct omap_dss_device *dssdev,
			struct omap_video_timings *timings)
{
	/* workaround to exclude w/h from comparison (since they are not really
	 * part of timings)..
	return memcmp(&dssdev->panel.timings, timings, sizeof(*timings));
	 */
	struct omap_video_timings *ptimings = &dssdev->panel.timings;
	return ptimings->x_res != timings->x_res ||
			ptimings->hfp != timings->hfp ||
			ptimings->hsw != timings->hsw ||
			ptimings->hbp != timings->hbp ||
			ptimings->y_res != timings->y_res ||
			ptimings->vfp != timings->vfp ||
			ptimings->vsw != timings->vsw ||
			ptimings->vbp != timings->vbp;
}
