...
int dss_init(struct platform_device *pdev)
{
	...
	/*
	 * In DSI CMD mode,can not use DISPC_CONTROL.LCDENABLE
	 * to check if bootloader enable LCD, this bit reset
	 * at the end of frame transfer. DISPC_CONTROL = 00018b08
	 * When code support video mode, need to add check
	 */
	skip_init = true;
#if 0
	/* DISPC_CONTROL */
...
fail1:
	iounmap(dss.base);
	dss_clk_disable(DSS_CLK_ICK | DSS_CLK_FCK1 |
		 DSS_CLK_FCK2 | DSS_CLK_54M | DSS_CLK_96M);
fail0:
	return r;
}
