...
struct omap_hdmi_data {
	...
	int active;
	struct mutex av_mutex; /* to sync audio and video operation */
};
...
static void hdmi_hpd_notifier(int state, void *data)
{
	...
	mutex_lock(&hdmi_data->av_mutex);
	substream = hdmi_data->substream;
	if (state) {
		hdmi_data->active = 1;
	} else {
		if (substream)
	...
	mutex_unlock(&hdmi_data->av_mutex);
}
...
static __devinit int omap_hdmi_probe(struct platform_device *pdev)
{
	...
	mutex_init(&hdmi_data.av_mutex);
	notifier->hpd_notifier = NULL;
	notifier->pwrchange_notifier = hdmi_hpd_notifier;
