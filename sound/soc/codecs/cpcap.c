...
static struct regulator *vaudio;
...
{
	int err;
	unsigned short val, mask, value = 0, value_rxsdoa = 0, value_rxepoa = 0;
	...
	if (val && strstr(name, "USBD")) {
		reg = CPCAP_REG_USBC2;
		value = CPCAP_BIT_EMUMODE2 | CPCAP_BIT_EMUMODE0;
		mask = CPCAP_BIT_UARTMUX1 | CPCAP_BIT_UARTMUX0 |
...
void cpcap_audio_init(struct snd_soc_codec *codec)
{
	...
	vaudio = regulator_get(NULL, "vaudio");
	if (IS_ERR(vaudio))
...
static int cpcap_add_widgets(struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_ignore_suspend(codec, "MIC1R");
	snd_soc_dapm_ignore_suspend(codec, "MICHS");
	snd_soc_dapm_ignore_suspend(codec, "MICEMU");
	snd_soc_dapm_ignore_suspend(codec, "MICEXTR");
	snd_soc_dapm_ignore_suspend(codec, "MIC2L");
	snd_soc_dapm_ignore_suspend(codec, "MICEXTL");

	snd_soc_dapm_ignore_suspend(codec, "LDSPR");
	snd_soc_dapm_ignore_suspend(codec, "LDSPL");
	snd_soc_dapm_ignore_suspend(codec, "HSR");
	snd_soc_dapm_ignore_suspend(codec, "HSL");
	snd_soc_dapm_ignore_suspend(codec, "LINER");
	snd_soc_dapm_ignore_suspend(codec, "LINEL");
	snd_soc_dapm_ignore_suspend(codec, "EMUSPKR");
	snd_soc_dapm_ignore_suspend(codec, "EMUSPKL");
	snd_soc_dapm_ignore_suspend(codec, "EP");

	snd_soc_dapm_ignore_suspend(codec, "ADC Right");
	snd_soc_dapm_ignore_suspend(codec, "ADC Left");

	snd_soc_dapm_ignore_suspend(codec, "AIFIN Voice");
	snd_soc_dapm_ignore_suspend(codec, "AIFIN Multimedia");
	snd_soc_dapm_ignore_suspend(codec, "AIFIN ExternalPGA");
			 ...
			 struct snd_soc_dai *dai)
{
	...
	if (state->stdac_strm_cnt == 0 &&
		...
		regulator_set_mode(vaudio, REGULATOR_MODE_NORMAL);
